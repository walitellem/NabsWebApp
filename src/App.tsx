        ...
        async (firebaseUser) => {
          ...
          if (firebaseUser) {
            try {
              ...
              try {
                // If we already have a valid session in localStorage, immediately unblock the UI 
                // while we sync in the background
                if (localStorage.getItem('nabslodge_active_session')) {
                  setIsAuthReady(true);
                }

                const userDocSnap = await safeFirestoreOp(() => getDoc(doc(db, 'users', uid)), null, 2000);
                if (userDocSnap?.exists()) {
                  docId = userDocSnap.id;
                } else if (cleanEmail) {
                  // Fallback query by email if doc by UID was created manually under another ID
                  const q = query(collection(db, 'users'), where('email', '==', cleanEmail));
                  const qSnap = await safeFirestoreOp(() => getDocs(q), null, 2000);
              ...
              // Set a safety timeout so we don't hang forever waiting for Firestore
              const authReadyTimeout = setTimeout(() => {
                console.warn("Auth observer timed out waiting for Firestore snapshot. Forcing auth ready.");
                setIsAuthReady(true);
              }, 5000);
