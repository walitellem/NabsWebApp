...
import { User, Room, Booking, AuditLog, Branch, RoomStatus, BookingStatus, PaymentStatus, Role, StaffUpdateInput, GlobalSettings, HandoverRecord, HandoverItemBreakdown, DrinkItem, DrinkSale, DrinkRestockRequest } from './types';
...
const DRINK_RESTOCK_REQUESTS_KEY = 'nabslodge_drink_restock_requests';
...
export const getDrinkRestockRequests = (): DrinkRestockRequest[] => {
  const data = localStorage.getItem(DRINK_RESTOCK_REQUESTS_KEY);
  if (!data) return [];
  try {
    const parsed: DrinkRestockRequest[] = JSON.parse(data);
...
export const saveDrinkRestockRequests = (requests: DrinkRestockRequest[]) => {
  localStorage.setItem(DRINK_RESTOCK_REQUESTS_KEY, JSON.stringify(requests));
};

export const addDrinkRestockRequest = (req: Omit<DrinkRestockRequest, 'id' | 'createdAt' | 'status'>): DrinkRestockRequest => {
  const requests = getDrinkRestockRequests();
  const newReq: DrinkRestockRequest = {
    ...req,
    id: `restock_${generateId()}`,
    status: 'Pending',
    createdAt: getFormattedDateTime()
  };
  requests.unshift(newReq);
  saveDrinkRestockRequests(requests);
  return newReq;
};

export const updateDrinkRestockRequest = (
  id: string,
  status: 'Approved' | 'Rejected',
  resolvedBy: string,
  rejectionReason?: string
): DrinkRestockRequest | null => {
  const requests = getDrinkRestockRequests();
  ...
  requests[index] = {
    ...
    status,
    resolvedBy,
    resolvedAt: getFormattedDateTime(),
