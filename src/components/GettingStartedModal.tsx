...
export function GettingStartedModal({ isOpen, onClose, currentUser, isDarkMode }: GettingStartedModalProps) {
  ...
  return (
    <AnimatePresence>
      {isOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" key="ap-getting-started">
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
          ...
          <motion.div
            ...
            className={`relative w-full max-w-2xl rounded-[2rem] overflow-hidden transition-colors duration-300 ${
              isDarkMode 
                ? 'neu-raised-lg text-zinc-100' 
          ...
          >
            ...
            <div className="relative p-6 pb-4 border-b border-zinc-200/50 dark:border-zinc-800/50">
              <button 
                ...
                className={`absolute top-4 right-4 p-2 rounded-full transition-colors cursor-pointer ${
                  isDarkMode 
                    ? 'neu-button text-zinc-400 hover:text-zinc-200' 
            ...
            {/* Role Switcher tabs */}
            <div className={`flex gap-2 mt-4 p-1 rounded-xl max-w-xs ${isDarkMode ? 'neu-inset' : 'bg-slate-100'}`}>
              <button
                onClick={() => handleRoleChange('Manager')}
                className={`flex-1 py-1.5 rounded-lg text-xs font-bold transition-all cursor-pointer ${
                  selectedRole === 'Manager'
                    ? (isDarkMode ? 'neu-raised text-blue-400 font-extrabold' : 'bg-white text-slate-800 shadow-xs')
          ...
          <div className="p-6 md:p-8 min-h-[320px] flex flex-col justify-between overflow-y-auto max-h-[75vh] pr-4 custom-inset-scrollbar flex-1">
            <AnimatePresence mode="wait">
              <motion.div
                key={generateKey(selectedRole, currentStep, 'step')}
              ...
              >
                ...
                <div className="flex items-center gap-4">
                  <div className={`p-3.5 rounded-2xl flex items-center justify-center ${isDarkMode ? 'neu-raised-sm' : 'bg-slate-50 border border-slate-200/50'}`}>
                    {currentStepData.icon}
                  </div>
                  <div>
                    <span className={`text-[10px] font-mono tracking-wider font-bold uppercase px-2 py-0.5 rounded-full ${isDarkMode ? 'neu-inset text-blue-400 font-extrabold' : 'bg-blue-100 text-blue-700'}`}>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 mt-2">
                  {currentStepData.highlights.map((highlight, idx) => (
                    <div key={generateKey(currentStepData.title, idx, 'highlight')} className="flex items-start gap-2.5">
                ...
                <div className={`p-3.5 rounded-xl flex gap-3 ${isDarkMode ? 'neu-inset' : 'p-3.5 rounded-xl border bg-slate-50 border-slate-200'}`}>
              ...
              <div className="flex items-center justify-between sm:justify-end gap-6">
                ...
                <div className="flex gap-1.5">
                  {activeSteps.map((_, idx) => (
                    <button
                      key={generateKey('step-dot', idx, 'dot')}
                ...
                <div className="flex gap-2">
                  ...
                  <button
                    onClick={handleNext}
                    className={`px-4 py-1.5 text-white rounded-xl text-xs font-bold flex items-center gap-1 transition-all cursor-pointer ${
                      isDarkMode
                        ? 'bg-blue-600 hover:bg-blue-500 text-white neu-glow-blue'
