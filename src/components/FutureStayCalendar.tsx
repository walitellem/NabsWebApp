...
export const FutureStayCalendar: React.FC<FutureStayCalendarProps> = ({
  ...
  if (!selectedRoom) {
    return (
      <div id="future-stay-calendar-placeholder" className={`p-6 rounded-2xl text-center flex flex-col items-center justify-center min-h-[220px] ${
        isDarkMode ? 'neu-raised text-zinc-100' : 'bg-white border border-slate-200 text-slate-800'
      }`}>
  ...
  for (let i = 0; i < firstDayIndex; i++) {
    dayCells.push(<div key={generateKey(i, i, 'fut-empty')} className="h-9 w-9" />);
  }
  ...
  for (let day = 1; day <= totalDays; day++) {
    ...
    // A date is a valid Check-Out date if check-in is set, dateStr > futureCheckIn,
    // and all intermediate nights between check-in and dateStr are free.
    const isValidCheckOutTarget = (() => {
      if (!futureCheckIn) return false;
      const currentMs = getLocalDateMidnight(dateStr);
      const inMs = getLocalDateMidnight(futureCheckIn);
      if (currentMs <= inMs) return false;

      let check = new Date(inMs);
      while (check.getTime() < currentMs) {
        const y = check.getFullYear();
        const m = String(check.getMonth() + 1).padStart(2, '0');
        const d = String(check.getDate()).padStart(2, '0');
        if (blockedDates[`${y}-${m}-${d}`]) {
          return false;
        }
    ...
    const canSelect = !isPast && (!isLocked || isValidCheckOutTarget);
    ...
    } else if (isLocked && isValidCheckOutTarget) {
    ...
    dayCells.push(
      <button
        key={generateKey(`${currentDate.getFullYear()}-${currentDate.getMonth()}`, day, 'fut-day')}
        type="button"
        disabled={!canSelect}
      ...
      >
        ...
        {!isPast && (!isLocked || isValidCheckOutTarget) && (
  ...
  return (
    <div id="future-stay-calendar-root" className={`p-4 rounded-2xl flex flex-col space-y-3.5 ${
      isDarkMode ? 'neu-raised text-zinc-100' : 'bg-white border border-slate-200 text-slate-800 shadow-2xl'
    }`}>
      ...
      <div className="grid grid-cols-7 gap-1 text-center text-[10px] font-semibold text-zinc-400 dark:text-zinc-500">
        {weekdays.map((wd, idx) => (
          <div key={generateKey(wd, idx, 'wd')} className="h-5 flex items-center justify-center">
            {wd}
          </div>
        ))}
      </div>
