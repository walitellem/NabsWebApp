...
export const RoomBookingCalendar: React.FC<RoomBookingCalendarProps> = ({
  ...
  const selectDay = (day: number) => {
    ...
    if (isDatePast(targetDate) || isDateBooked(targetDate)) return;

    const targetTime = startOfDay(targetDate);

    // If checkInDate is not set, or both are set, start new selection
    if (!checkInDate || (checkInDate && checkOutDate)) {
      const checkInStr = formatDateTimeLocal(targetDate, 14, 0); // Default check-in time 14:00
      onDatesChange(checkInStr, '');
    } else {
      // Check-in date is set, but check-out is not
      ...
      if (targetTime > currentCheckInTime) {
        ...
        if (overlapsBooked) {
          // Restart selection on clicked date
        ...
        // Clicked a date before or equal to check-in; make it the new check-in date
        const checkInStr = formatDateTimeLocal(targetDate, 14, 0);
        onDatesChange(checkInStr, '');
      }
    }
  ...
  for (let day = 1; day <= totalDays; day++) {
    ...
    days.push(
      <button
        ...
        disabled={isPast}
        onClick={() => {
          if (!isPast && !isBooked) selectDay(day);
        }}
  ...
  return (
    <div className={`p-4 rounded-2xl border ${isDarkMode ? 'bg-zinc-900 border-zinc-800 text-white' : 'bg-white border-slate-200 text-slate-800'}`}>
      ...
      <div className="grid grid-cols-7 gap-1 text-center text-[10px] font-semibold text-zinc-400 dark:text-zinc-500 mb-2">
        {weekdays.map((wd) => (
          <div key={wd} className="h-5 flex items-center justify-center">{wd}</div>
        ))}
      </div>
