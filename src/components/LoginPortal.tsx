...
import { AuthDiagnosticView } from './AuthDiagnosticView';
...
export function LoginPortal({ onLoginSuccess, onForbidden, isDarkMode, onToggleTheme }: LoginPortalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await withLoading((async () => {
        ...
        if (isFirebaseConfigured) {
          try {
            ...
            // If Firebase Auth fails, we don't immediately throw for credential errors,
            // we allow it to fall back to the Firestore document password check 
            // for users who exist in the database but haven't been added to Firebase Auth yet.
        ...
        if (isFirebaseConfigured && firebaseUid) {
          try {
            const userDocSnap = await safeFirestoreOp(() => getDoc(doc(db, 'users', firebaseUid!)), null, 2000);
        ...
        if (isFirebaseConfigured && !firestoreData) {
          try {
            ...
            const querySnapshot = await safeFirestoreOp(() => getDocs(q), null, 2000);
        ...
        if (!authUser) {
          ...
          // If we reach here, the password is correct in Firestore/local, but the Firebase Auth user doesn't exist.
          // To prevent App.tsx's onAuthStateChanged from instantly logging the user out, we must create the Auth user.
          if (isFirebaseConfigured) {
            console.log("DEBUG: Valid credentials found in Firestore but not Auth. Creating Auth user...");
            try {
              const newUserCredential = await createUserWithEmailAndPassword(auth, cleanEmail, password);
              authUser = newUserCredential.user;
              firebaseUid = authUser.uid;
              
              // Also ensure we link the new UID back to the Firestore document if necessary
              // However, since Firestore handles queries by email, it will eventually sync.
              if (firestoreData && userId && userId !== firebaseUid) {
                 await safeFirestoreOp(() => updateDoc(doc(db, 'users', userId), { uid: firebaseUid }), null, 5000);
              }
            } catch (createErr: any) {
              console.warn("Failed to lazily create Auth user:", createErr);
              // If we can't create the user, they will likely get kicked out by App.tsx, but we proceed anyway
            }
          }
        }
      ...
      if (errStr.includes('deactivated') || errStr.includes('Access denied') || errStr.includes('IP restriction')) {
        ...
        errorMessage = 'Incorrect email or password. (' + errCode + ' - ' + errStr + ')';
      }
  ...
  return (
    <div className="bg-[#e6eaf0] dark:bg-[#0d1527] text-zinc-900 dark:text-zinc-50 transition-colors duration-200 min-h-screen w-full flex flex-col justify-between p-4 sm:p-6 md:p-8 relative overflow-hidden font-sans">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className={`p-2.5 rounded-xl flex items-center justify-center transition-all cursor-pointer ${
              isDarkMode 
                ? 'neu-button text-blue-400' 
                : 'bg-white border border-slate-200 text-blue-600 shadow-sm hover:bg-slate-50'
            }`}
          ...
          <div className={`hidden sm:flex items-center gap-2 rounded-full px-4 py-1.5 text-xs ${
            isDarkMode 
              ? 'neu-inset text-zinc-300' 
      ...
      <main className="w-full max-w-5xl mx-auto my-auto grid grid-cols-1 lg:grid-cols-12 gap-8 items-center z-10 py-8">
        ...
        <div className="lg:col-span-5">
          <motion.div 
            ...
            className={`p-6 sm:p-8 relative transition-all ${
              isDarkMode ? 'neu-raised-lg' : 'bg-white border border-slate-200 rounded-3xl shadow-2xl'
            }`}
          >
            <div className={`absolute top-0 right-8 -translate-y-1/2 px-3 py-1 text-[11px] font-mono uppercase tracking-widest ${
              isDarkMode ? 'neu-inset text-blue-400 font-bold' : 'bg-slate-100 border border-slate-200 text-slate-500 rounded-full'
            }`}>
              SECURE SSO
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div>
                <label className={`block text-xs font-mono uppercase tracking-wider mb-1.5 ${
                  isDarkMode ? 'text-zinc-400 font-bold' : 'text-slate-500'
                }`}>
                  Email Address
                </label>
                <div className="relative">
                  ...
                  <input
                    ...
                    disabled={isLocked}
                    className={`block w-full pl-10 pr-3 py-2.5 text-sm transition-all font-mono focus:outline-none disabled:opacity-50 disabled:cursor-not-allowed ${
                      isDarkMode 
                        ? 'neu-inset text-white placeholder-zinc-500 focus:ring-1 focus:ring-blue-500' 
                        : 'bg-slate-50 border border-slate-200 rounded-xl text-slate-900 placeholder-slate-400 focus:border-blue-500'
                    }`}
                  />
                </div>
              </div>
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    placeholder="••••••••••••"
                    disabled={isLocked}
                    className={`block w-full pl-10 pr-10 py-2.5 text-sm transition-all font-mono focus:outline-none disabled:opacity-50 disabled:cursor-not-allowed ${
                      isDarkMode 
                        ? 'neu-inset text-white placeholder-zinc-500 focus:ring-1 focus:ring-blue-500' 
                        : 'bg-slate-50 border border-slate-200 rounded-xl text-slate-900 placeholder-slate-400 focus:border-blue-500'
                    }`}
                  ...
                  <button
                    ...
                    className="absolute inset-y-0 right-0 pr-3.5 flex items-center text-slate-400 dark:text-zinc-400 hover:text-slate-600 dark:hover:text-zinc-200"
                  >
            ...
            <button
              ...
              className={`mt-6 w-full py-2.5 rounded-xl border border-dashed flex items-center justify-center gap-2 text-[11px] font-mono transition-colors ${
                isDarkMode 
                  ? 'border-zinc-700 text-zinc-400 hover:bg-zinc-800 hover:text-blue-400' 
                  : 'border-slate-300 text-slate-500 hover:bg-slate-100 hover:text-blue-600'
              }`}
            >
            ...
            <div className={`mt-6 pt-4 border-t ${isDarkMode ? 'border-zinc-800/80' : 'border-slate-100'} text-xs space-y-3`}>
              ...
              <div className={`p-3 rounded-2xl ${isDarkMode ? 'neu-inset' : 'bg-slate-50 border border-slate-100'} space-y-2`}>
                  ...
                  <div className="space-y-1.5">
                    <input
                      ...
                      className={`block w-full px-2.5 py-1.5 rounded-lg text-xs font-mono focus:outline-none ${
                        isDarkMode ? 'neu-inset text-white' : 'bg-white border border-slate-200 text-slate-700'
                      }`}
                    />
      ...
      {showDiagnostics && (
        <AuthDiagnosticView onClose={() => setShowDiagnostics(false)} />
      )}
    </div>
  );
}
