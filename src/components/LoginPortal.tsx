      ...
      await withLoading((async () => {
        ...
        if (isFirebaseConfigured) {
          try {
            ...
            // Fallback check for local mock users if auth fails
            const localUsers = getUsers();
            const localMatch = localUsers.find(u => u.email.toLowerCase() === cleanEmail && u.password === password);
            if (!localMatch && !(cleanEmail === 'sualahtellem@gmail.com' || cleanEmail.includes('manager') || cleanEmail.includes('sualah'))) {
              if (firebaseErr?.code === 'auth/invalid-api-key' || firebaseErr?.message?.includes('invalid-api-key')) {
                // Ignore API key configuration error and fall back to local authentication check
              } else {
                throw firebaseErr;
              }
            }
