...
export const EditBookingModal: React.FC<EditBookingModalProps> = ({
  ...
  const isSplitAmountChanged =
    parseFloat(splitCashAmount || '0') !== initialSplitCash ||
    parseFloat(splitMomoAmount || '0') !== initialSplitMomo;
  ...
  const isRevenueAlteringChanged =
    ...
    isSplitAmountChanged ||
    proposedPaymentMethod === 'Split (Cash + Momo)';

  const hasAnyChange =
    isGuestDetailsOnlyChanged ||
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <motion.div
        ...
        className={`relative rounded-[2rem] p-6 w-full max-w-xl shadow-2xl flex flex-col max-h-[90vh] ${
          isDarkMode ? 'neu-raised-lg text-zinc-100' : 'bg-white border border-slate-200 text-slate-900'
        }`}
      >
        {/* Header */}
        <div className="flex items-center justify-between pb-4 border-b border-zinc-200 dark:border-zinc-800/50 shrink-0">
          <div className="flex items-center gap-2.5">
            <div className={`p-2.5 rounded-2xl ${isDarkMode ? 'neu-raised-sm text-blue-400' : 'bg-blue-600/10 text-blue-500'}`}>
          ...
          <button
            onClick={onClose}
            className={`p-1.5 rounded-xl transition-colors ${isDarkMode ? 'neu-button text-zinc-400 hover:text-zinc-200' : 'hover:bg-zinc-100 text-zinc-400'}`}
          >
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-5 pt-4 pr-3 overflow-y-auto max-h-[75vh] custom-inset-scrollbar flex-1">
          ...
          <div className={`p-4 rounded-2xl ${isDarkMode ? 'neu-inset' : 'bg-slate-50 border border-slate-200'}`}>
          ...
          <div className={`p-4 rounded-2xl ${isDarkMode ? 'neu-inset' : 'bg-slate-50 border border-slate-200'}`}>
            ...
            <div className="space-y-3.5">
              <div>
                ...
                >
                  {rooms.map((r, idx) => (
                    <option key={generateKey(r.id || r.roomNumber, idx, 'edit-room-opt')} value={r.id}>
