...
export const TransferRoomModal: React.FC<TransferRoomModalProps> = ({
  ...
  const theme = {
    bg: isDarkMode ? 'neu-raised-lg text-white' : 'bg-white border border-slate-200 text-slate-900',
    panel: isDarkMode ? 'neu-inset' : 'bg-slate-50 border border-slate-200',
    input: isDarkMode ? 'neu-inset text-white focus:ring-1 focus:ring-blue-500' : 'bg-white border border-slate-300 text-slate-900',
    text: isDarkMode ? 'text-zinc-300' : 'text-slate-600',
    border: isDarkMode ? 'border-zinc-800/50' : 'border-slate-200',
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <motion.div
        ...
        className={`relative w-full max-w-lg max-h-[90vh] flex flex-col rounded-[2rem] shadow-2xl overflow-hidden ${theme.bg}`}
      >
            ...
            <div>
              ...
              >
                ...
                {availableRooms.map((r, idx) => (
                  <option key={generateKey(r.id || r.roomNumber, idx, 'transfer-rm')} value={r.id}>Room {r.roomNumber} - {r.roomType} (GH₵{r.price}/night)</option>
                ))}
              </select>
            </div>
