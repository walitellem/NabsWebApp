...
export const DrinkSalesLedger: React.FC<DrinkSalesLedgerProps> = ({
  ...
  useEffect(() => {
    setCurrentPage(1);
  }, [searchQuery, orderTypeFilter, paymentStatusFilter, receptionistShiftFilter, managerStaffFilter, managerBranchFilter, datePreset, customStartDate, customEndDate]);
  ...
  return (
    <div className="space-y-6 w-full">
      ...
      <div className={`p-6 rounded-3xl transition-all ${
        isDarkMode ? 'neu-raised-lg text-zinc-100' : 'bg-white border border-slate-200 shadow-xs'
      }`}>
        ...
        <div className="space-y-4 pt-2 border-t border-zinc-200 dark:border-zinc-800">
              ...
              {!isManager ? (
                ...
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  >
                    ...
                    {uniqueStaffOptions.map((staff, idx) => (
                      <option key={generateKey(staff.id || staff.name || idx, idx, 'staff-opt')} value={staff.id}>
          ...
          <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-3 pt-2">
            ...
            <div className="flex items-center gap-1.5 flex-wrap">
              ...
              {(['today', 'yesterday', 'this_week', 'this_month', 'all_time', 'custom'] as const).map((preset, idx) => (
                <button
                  key={generateKey(preset, idx, 'preset')}
          ...
          <AnimatePresence>
            {datePreset === 'custom' && (
<motion.div key="ap-motion-div-laz8up"
...
<button key="ap-button-pvvx8n"
      ...
      <div className={`rounded-3xl overflow-hidden transition-all ${
        isDarkMode ? 'neu-raised-lg' : 'bg-white border border-slate-200 shadow-xs'
      }`}>
          ...
          <table className="w-full text-left text-xs">
            ...
            <tbody className={`divide-y ${isDarkMode ? 'divide-zinc-800/60' : 'divide-slate-100'}`}>
              {filteredSales.length === 0 ? (
                ...
                filteredSales.slice((currentPage - 1) * pageSize, currentPage * pageSize).map((sale, idx) => {
                  ...
                  return (
                    <tr 
                      key={generateKey(sale.id || sale.serialNumber || idx, idx, 'sale-row')} 
                        ...
                        {sale.items && sale.items.length > 0 ? (
                          <div className="space-y-0.5">
                            {sale.items.map((item, idx) => (
                              <div key={generateKey(item.drinkId, idx, 'item')} className="text-xs font-semibold">
                                {item.drinkName} 
...
<div key="ap-div-93dhss" className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
            <motion.div
                  ...
                  <div className="space-y-1.5">
                    {selectedSaleForDetails.items && selectedSaleForDetails.items.length > 0 ? (
                      selectedSaleForDetails.items.map((item, idx) => {
                        ...
                        return (
<div 
                            key={generateKey(selectedSaleForDetails.id, item.drinkId || item.drinkName || idx, 'sale-item')} 
      ...
      <AnimatePresence>
        {saleToSettle && (
<div key="ap-div-hg45z" className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
            <motion.div
              initial={{ scale: 0.95, opacity: 0 }}
              animate={{ scale: 1, opacity: 1 }}
              exit={{ scale: 0.95, opacity: 0 }}
...
<div key="ap-div-uvwvac" className="flex justify-between items-center mb-1">
