...
import { syncAndReconcileBookings, fetchFreshBookingsFromFirestore, sortBookingsDescending } from '../utils/bookingSync';
...
function sanitizeClonedDoc(clonedDoc: Document) {
  ...
  // Force clean desktop width (800px) or thermal width (380px), white background, and crisp layout for printed/downloaded invoice & receipt sheets
  ...
  printSheets.forEach((sheetEl) => {
    ...
    const isThermal = sheet.classList.contains('thermal-sheet') || 
                      sheet.className.includes('thermal') || 
                      sheet.parentElement?.className.includes('390px');

    if (isThermal) {
      sheet.style.width = '380px';
      sheet.style.minWidth = '380px';
      sheet.style.maxWidth = '380px';
      sheet.style.padding = '16px';

      // Thermal 80mm: Force header row to stack vertically so reference block stays within margin
      ...
      headerRights.forEach((right) => {
        ...
        rightEl.style.borderTop = '1px dashed #cbd5e1';
        rightEl.style.paddingTop = '8px';
        rightEl.style.marginTop = '4px';
      });
      ...
      // Force header row to stay flex-row with space-between on A4 PDF capture
      const headerRows = sheet.querySelectorAll('.invoice-header-row');
      headerRows.forEach((row) => {
        const rowEl = row as HTMLElement;
        rowEl.style.display = 'flex';
      ...
      // Force 2-column grid layout for guest info & stay schedule in captured A4 PDF
      const gridCols = sheet.querySelectorAll('.grid-cols-1, .grid-cols-2');
      gridCols.forEach((grid) => {
        const gridEl = grid as HTMLElement;
        gridEl.style.display = 'grid';
    ...
    // Constrain all logos inside the cloned sheet
...
export function ReceptionistDashboard({ currentUser, onLogout, isDarkMode, onToggleTheme, onOpenTutorial }: ReceptionistDashboardProps) {
  ...
  const triggerPrint = (elementId?: string, title: string = 'Hotel Document') => {
    ...
    if (targetEl) {
      const isThermal = invoicePrintPaperSize === 'thermal';
      
      try {
        const headStyles = Array.from(document.querySelectorAll('style, link[rel="stylesheet"]'))
          .map(el => {
            if (el.tagName.toLowerCase() === 'style') {
              const text = sanitizeCssColors(el.textContent || '')
          ...
          printWin.document.write(`
            ...
            <html>
                ...
                <style>
                  * { 
                  ...
                  @page {
                    ...
                    margin: ${isThermal ? '0' : '12mm 10mm'};
                  }
                  body {
                    font-family: system-ui, -apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, sans-serif;
                    background: #ffffff !important;
                    color: #000000 !important;
                    padding: ${isThermal ? '8px' : '16px'} !important;
                    margin: 0 !important;
                  }
                  #print-sheet-root, #print-sheet-root * {
                    visibility: visible !important;
                  }
                  #print-sheet-root .invoice-header-row {
                  ...
                  #print-sheet-root .invoice-header-right {
                    text-align: right !important;
                  }
                  ${!isThermal ? `
                    #print-sheet-root .grid-cols-2 {
                      display: grid !important;
                      grid-template-columns: repeat(2, minmax(0, 1fr)) !important;
                      gap: 16px !important;
                    }
                  ` : `
                    #print-sheet-root .grid {
                      display: flex !important;
              ...
              <body class="bg-white text-slate-900">
                <div id="print-sheet-root" style="width: ${isThermal ? '76mm' : '100%'}; margin: 0 auto; background: #ffffff;">
                  ${targetEl.innerHTML}
                </div>
                <script>
      ...
      printStyle.innerHTML = `
        @media print {
          * {
          ...
          @page {
            size: ${isThermal ? '80mm auto' : 'A4 portrait'};
            margin: ${isThermal ? '0' : '15mm 10mm 15mm 10mm'};
          }
          ...
          #${targetEl.id} {
            ...
            left: 0 !important;
            top: 0 !important;
            width: ${isThermal ? '76mm !important' : '100% !important'};
            margin: 0 !important;
            padding: ${isThermal ? '8px !important' : '0 !important'};
            background: #ffffff !important;
            color: #000000 !important;
  ...
  const branchBookings = useMemo(() => {
    const list = bookings.filter(b => b.branch === branch || !b.branch);
    return sortBookingsDescending(list);
  }, [bookings, branch]);
  ...
  const [invoicePrintPaperSize, setInvoicePrintPaperSize] = useState<'thermal' | 'a4'>('a4');
  ...
  const [futureStatusFilter, setFutureStatusFilter] = useState<'All' | 'Active' | 'Pending Review' | 'Checked-In' | 'No Show'>('All');
  ...
  const todaysArrivals = useMemo(() => {
    ...
    const activeList = bookings && bookings.length > 0 ? bookings : currentBookings;
    return activeList.filter(b => {
      if (!b.checkInDate) return false;
      const bInStr = b.checkInDate.split('T')[0];
      const isToday = bInStr === todayStr;

      const bBranch = (b.branch || b.lodgeBranch || '').toLowerCase();
      const targetBranch = (branch || '').toLowerCase();
      const matchesBranch = !branch || bBranch.includes(targetBranch) || targetBranch.includes(bBranch);

      const statusStr = String(b.status || '').toLowerCase().replace(/[^a-z0-9]/g, '');
      const isPendingArrival = !['checkedin', 'checkedout', 'cancelled', 'noshow'].includes(statusStr) && !b.checkedIn && !(b as any).noShow;
  ...
  const [lastHistorySyncTime, setLastHistorySyncTime] = useState<string>('Live');

  const handleManualSyncHistory = async () => {
    setIsSyncingHistory(true);
    try {
      const fresh = await fetchFreshBookingsFromFirestore();
      setBookings(fresh);
      setLastHistorySyncTime(new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }));
      addToast('Booking History Refreshed', 'success', `Successfully synchronized ${fresh.length} booking records.`, 3000);
    } catch (e) {
      console.warn("Manual history sync error:", e);
      addToast('Sync Warning', 'warning', 'Could not refresh from server. Displaying cached records.', 3500);
    } finally {
  ...
  useEffect(() => {
    setHistoryPage(1);
  }, [historySearch, historyStatusFilter, historyPaymentFilter, historyStartDate, historyEndDate]);

  // Reservations Table Pagination
  const [reservationTablePage, setReservationTablePage] = useState(1);
  const [reservationTablePageSize, setReservationTablePageSize] = useState(25);

  // Rooms Table View Pagination
  const [roomTableViewPage, setRoomTableViewPage] = useState(1);
  const [roomTableViewPageSize, setRoomTableViewPageSize] = useState(25);

  // Today's Bookings Table Pagination
  const [todayBookingPage, setTodayBookingPage] = useState(1);
  const [todayBookingPageSize, setTodayBookingPageSize] = useState(25);

  useEffect(() => {
    setTodayBookingPage(1);
  }, [todayBookingSearch]);
  ...
  }, [branchBookings, historyStatusFilter, historyPaymentFilter, historyStartDate, historyEndDate, historySearch]);

  const totalHistoryPages = Math.max(1, Math.ceil(filteredHistoryBookings.length / historyPageSize));
  ...
  const paginatedHistoryBookings = useMemo(() => {
    ...
    return filteredHistoryBookings.slice(startIndex, startIndex + historyPageSize);
  }, [filteredHistoryBookings, safeHistoryPage, historyPageSize]);
  ...
  const handleCheckOut = async () => {
    ...
    try {
      ...
      // Update Firestore Booking with full settlement guarantee
      ...
      try {
        const canvas = await executeWithOklchSafeStyles(async () => {
          return await html2canvas(element, { 
            ...
            scrollX: 0,
            scrollY: 0,
            windowWidth: isThermal ? 420 : 1200,
        ...
        if (isThermal) {
          ...
          const thermalHeightMm = Math.max(120, (canvas.height * thermalWidthMm) / canvas.width);
          pdf = new jsPDF('p', 'mm', [thermalWidthMm, thermalHeightMm]);
        ...
        console.warn("Canvas Receipt generation failed, falling back to system print dialogue:", error);
      }
    }

    triggerPrint('checkout-receipt-card', `Checkout Receipt - Room ${selectedBooking?.roomNumber || 'Booking'}`);
  };
  ...
  const handleDownloadInvoicePDF = async () => {
    ...
    if (element) {
      try {
        ...
        console.warn("Canvas PDF generation failed, falling back to system print dialogue:", error);
      }
    }

    triggerPrint('print-invoice-sheet', `Invoice Ref: INV-${invoiceBooking?.id?.replace('book_', '').toUpperCase() || 'Booking'}`);
  };
  ...
  const handleDownloadWalkInPDF = async () => {
    ...
    triggerPrint('walkin-invoice-card', `WalkIn Invoice - ${walkInReceiptData?.serialNumber || 'Activity'}`);
  };
  ...
  useEffect(() => {
    ...
    try {
      ...
      unsubBookings = onSnapshot(bookingsQ, async (snapshot) => {
        const fetched = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() } as Booking));
        const reconciled = await syncAndReconcileBookings(fetched);
        setBookings(reconciled);
        setLastHistorySyncTime(new Date().toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }));
        setIsLoadingData(false);
      }, (err) => {
        console.warn("Firestore bookings snapshot listener error:", err);
        setBookings(sortBookingsDescending(getBookings()));
        setIsLoadingData(false);
      });
  ...
  const logRoomRevenue = async (data: {
    ...
    try {
      ...
      const methodSuffix = (data.paymentMethod || 'cash').toLowerCase().replace(/\s+/g, '_');
      const revId = data.customId || (
        data.revenueType === 'ExtensionFee' 
          ? `rev_ext_${data.bookingId}_${Date.now()}`
          : `rev_${typeKey}_${data.bookingId}_${methodSuffix}`
      );
      const userAssignedBranch = currentUser.assignedBranch || currentUser.branch || branch;
      const newRevDoc = {
        id: revId,
  ...
  const handleBookFutureStaySubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Auto-trigger invoice print modal
      setInvoiceBooking(newBooking);
      setInvoiceType('CheckIn');
      setShowPrintInvoiceModal(true);
  ...
  const handleMarkNoShow = async (booking: Booking) => {
    ...
    try {
      ...
      await safeSetDoc(doc(db, 'bookings', booking.id), {
        status: 'Cancelled',
        noShow: true,
      ...
      <>
        {parts.map((part, i) => 
          part.toLowerCase() === query.toLowerCase() 
            ? <span key={generateKey(query, i, 'highlight')} className="bg-purple-500/30 text-purple-600 dark:text-purple-400 font-black px-0.5 rounded">{part}</span> 
            : <span key={generateKey(query, i, 'highlight')}>{part}</span>
        )}
      </>
    );
  };
  ...
  return (
    <div id="receptionist-dashboard-container" tabIndex={-1} className={`${theme.bg} transition-colors duration-200 min-h-screen w-full flex flex-col md:flex-row font-sans outline-none`}>
      ...
      <aside className={`h-screen max-h-screen flex flex-col fixed inset-y-0 left-0 z-50 w-64 bg-[#e6eaf0] dark:bg-[#131e36] neu-raised rounded-none border-r border-zinc-200/50 dark:border-zinc-800/50 md:sticky md:top-0 transition-transform duration-300 ease-in-out shrink-0 ${
        isSidebarOpen 
          ? 'translate-x-0 flex' 
        ...
        <div className="p-6 border-b border-zinc-200/50 dark:border-zinc-800/50">
          ...
          <div className="px-3 py-2 text-xs font-mono neu-inset text-zinc-700 dark:text-zinc-300">
            <div className="text-[10px] uppercase tracking-widest font-bold mb-0.5 text-zinc-500 dark:text-zinc-400">Receptionist</div>
            <div className="truncate font-semibold" title={currentUser?.name || ''}>{currentUser?.name || 'User'}</div>
          </div>
        </div>

        <nav 
          className="flex-1 overflow-y-auto overscroll-contain py-4 px-4 space-y-2"
          style={{ WebkitOverflowScrolling: "touch" }}
        >
          <button
            onClick={() => { setActiveTab('rooms'); setIsSidebarOpen(false); }}
            className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl text-xs font-bold transition-all whitespace-nowrap cursor-pointer ${
              activeTab === 'rooms'
                ? 'neu-inset text-blue-600 dark:text-blue-400 font-extrabold'
                : 'neu-button text-zinc-700 dark:text-zinc-300 hover:text-blue-600 dark:hover:text-blue-400'
            }`}
          >
          ...
          <button
            ...
            className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl text-xs font-bold transition-all whitespace-nowrap cursor-pointer ${
              activeTab === 'quickCalendar'
                ...
                : 'neu-button text-zinc-700 dark:text-zinc-300 hover:text-indigo-600 dark:hover:text-indigo-400'
            }`}
          >
          ...
          {isHistoryUnlocked && (
            <button
              ...
              className={`w-full flex items-center justify-between px-4 py-3 rounded-xl text-xs font-bold transition-all whitespace-nowrap cursor-pointer ${
                activeTab === 'history'
                  ? 'neu-inset text-blue-600 dark:text-blue-400 font-extrabold'
                  : 'neu-button text-zinc-700 dark:text-zinc-300 hover:text-blue-600 dark:hover:text-blue-400'
              }`}
          ...
          <button
            ...
            className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl text-xs font-bold transition-all whitespace-nowrap cursor-pointer ${
              activeTab === 'drinks'
                ? 'neu-inset text-purple-600 dark:text-purple-400 font-extrabold'
                : 'neu-button text-zinc-700 dark:text-zinc-300 hover:text-purple-600 dark:hover:text-purple-400'
            }`}
          >
          ...
          <div className="mt-6 border-t pt-4 border-zinc-200/50 dark:border-zinc-800/50">
            <HeartbeatIndicator 
              role="Receptionist" 
              branch={branch}
        ...
        <div className="p-4 border-t border-zinc-200/50 dark:border-zinc-800/50 flex flex-col gap-2 transition-colors duration-300">
          <button
            onClick={() => { onOpenTutorial?.(); setIsSidebarOpen?.(false); }}
            className="flex items-center justify-center gap-2 w-full py-2.5 rounded-xl text-xs font-bold transition-all cursor-pointer neu-button text-blue-600 dark:text-blue-400"
          >
          ...
          <button
            ...
            className="flex items-center justify-center gap-2 w-full py-2.5 rounded-xl text-xs font-bold transition-all cursor-pointer neu-button text-zinc-700 dark:text-zinc-300"
          >
      ...
      {/* Main Content Area */}
      <main className="flex-1 p-4 md:p-6 bg-[#e6eaf0] dark:bg-[#0d1527] min-h-screen overflow-y-auto transition-colors duration-300 flex flex-col">
        
        {/* Mobile Header (only visible on md-) */}
        <header className="w-full h-16 neu-raised flex items-center justify-between px-4 sticky top-0 z-30 md:hidden mb-4 rounded-2xl">
          <div className="flex items-center gap-3">
            <button 
              onClick={() => setIsSidebarOpen(!isSidebarOpen)}
              className="p-2 neu-button flex items-center justify-center cursor-pointer text-zinc-700 dark:text-zinc-300"
            >
          ...
          <div className="flex flex-row items-center gap-2">
            <button onClick={onOpenTutorial} title="Onboarding Guide" className="p-1.5 neu-button text-blue-600 dark:text-blue-400">
              <HelpCircle className="w-4 h-4" />
            </button>
            <button onClick={onToggleTheme} className="p-1.5 neu-button text-blue-600 dark:text-blue-400">
              {isDarkMode ? <Sun className="w-4 h-4" /> : <Moon className="w-4 h-4" />}
            </button>
            <button onClick={() => setShowSignOutModal(true)} className="p-1.5 neu-button text-zinc-700 dark:text-zinc-300 hover:text-red-600 dark:hover:text-red-400">
              <LogOut className="w-4 h-4" />
            </button>
          </div>
        </header>
        ...
        <div className="max-w-7xl mx-auto w-full mb-6">
          <div className="neu-raised-lg p-6">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
              ...
              <div className="flex flex-wrap gap-2">
                <button
                  ...
                  className={`px-4 py-2.5 font-bold rounded-xl text-xs transition-all cursor-pointer flex items-center gap-2 relative ${
                    todaysArrivals.length > 0
                      ? 'bg-amber-500 hover:bg-amber-600 text-slate-950 neu-glow-amber'
                      : 'neu-button text-zinc-700 dark:text-zinc-300'
                  }`}
            ...
            <div className="flex flex-wrap gap-2 mt-4">
              {(['All', 'Active', 'Pending Review', 'Checked-In', 'No Show'] as const).map((st, idx) => (
                <button
                  key={generateKey(st, idx, 'item')}
              ...
              {(() => {
                ...
                return (
                  <div>
                    <div className="overflow-x-auto">
                      <table className="w-full text-left border-collapse text-xs">
                        ...
                        <tbody className="divide-y divide-zinc-100 dark:divide-zinc-850">
                          {filtered.slice((reservationTablePage - 1) * reservationTablePageSize, reservationTablePage * reservationTablePageSize).map((b, idx) => {
                            ...
                            return (
                              <tr key={generateKey(b.id, idx, 'active-row')} className="hover:bg-zinc-50 dark:hover:bg-zinc-900/40 transition-colors">
                                ...
                                <td className="py-3.5 px-2">
                                  ...
                                  <div className="flex items-center gap-1.5 mt-0.5 text-[10px]">
                                    ...
                                    <span className="text-zinc-400 font-mono">{b.guestContact || (b as any).guestPhone || '—'}</span>
                                  </div>
                                </td>
                                    ...
                                    {(b.status === 'Confirmed' || b.status === 'Pending') && (
                                      <>
                                        ...
                                        <button
                                          ...
                                          className="px-2.5 py-1.5 bg-indigo-600 hover:bg-indigo-500 text-white font-bold rounded-lg text-[10px] transition-all cursor-pointer flex items-center gap-1"
                                        >
                    ...
                    <TablePagination
                      currentPage={reservationTablePage}
                      totalItems={filtered.length}
                      pageSize={reservationTablePageSize}
                      onPageChange={setReservationTablePage}
                      onPageSizeChange={setReservationTablePageSize}
                      isDarkMode={isDarkMode}
            ...
            >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                {dueSoonBookings.map((b, idx) => {
                  ...
                  return (
                    <div 
                      key={generateKey(b.id, idx, 'overdue-checkout')}
                      ...
                      <tbody className="divide-y divide-slate-100 dark:divide-zinc-850">
                        {isLoadingData ? (
                          Array.from({ length: 5 }).map((_, idx) => (
                            <tr key={generateKey(idx, idx, 'skel-row')} className="animate-pulse">
                          ...
                          .map((room, idx) => {
                            ...
                            return (
<tr key={generateKey((room.id || room.roomNumber) + '-' + (room.branch || ''), idx, 'room-tr')} className={`transition-colors ${radarClasses ? radarClasses : theme.tableRowHover}`}>
                                ...
                                <td className="p-4">
                                  ...
                                  {isOccupied && activeBooking && (
                                    <>
                                      ...
                                      {(activeBooking.guestContact || (activeBooking as any).guestPhone) && (
                                ...
                                <td className="p-4">
                                  <div className="flex flex-wrap gap-1 max-w-[200px]">
                                    {room.amenities.slice(0, 3).map((a, idx) => (
                                      <span key={generateKey(a, idx, 'amenity')} className={`text-[9px] font-semibold px-1.5 py-0.5 rounded border ${isDarkMode ? 'bg-zinc-950 text-emerald-500 border-emerald-900/30' : 'bg-white text-emerald-600 border-emerald-200'}`}>
                                        {a}
                                      </span>
                                    ))}
                                    {room.amenities.length > 3 && (
<span key="ap-span-065g8i" className={`text-[9px] font-semibold px-1.5 py-0.5 rounded border ${isDarkMode ? 'bg-zinc-950 text-emerald-500 border-emerald-900/30' : 'bg-white text-emerald-600 border-emerald-200'}`}>+{room.amenities.length - 3} more</span>
                                    )}
                                  </div>
                                </td>
...
<div key="ap-div-vmz7m9" className="flex gap-1.5 justify-end">
                                            <button
                                              onClick={() => {
                                                if (activeBooking) {
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {isLoadingData ? (
                    Array.from({ length: 6 }).map((_, idx) => (
                      <div key={generateKey(idx, idx, 'skel-card')} className={`${theme.card} p-5 space-y-4 animate-pulse`}>
                    ...
                    .map((room, idx) => {
                      ...
                      return (
<div 
                          key={generateKey((room.id || room.roomNumber) + '-' + (room.branch || ''), idx, 'room-grid')}
                          className={`${theme.card} p-5 space-y-4 relative overflow-hidden transition-all duration-300 hover:border-blue-500/50 ${radarBorder}`}
                        >
                          {/* Header */}
                          <div className="flex justify-between items-start">
                            <div>
                          ...
                          {/* Notifications */}
                          {isCleaning && (
<div key="ap-div-5i1sts" className="bg-amber-500/10 border border-amber-500/20 text-amber-600 dark:text-amber-400 text-xs font-bold px-3 py-2 rounded-xl flex items-center gap-2">
                          ...
                          {isMaintenance && (
<div key="ap-div-wmf1fa" className="bg-red-500/10 border border-red-500/20 text-red-600 dark:text-red-400 text-xs font-bold px-3 py-2 rounded-xl flex items-center gap-2">
                          ...
                          {isOccupied && activeBooking && (() => {
                            ...
                            if (hours <= 2 || isOverdue) {
                              return (
<div key="ap-div-gg9ojm" className={`bg-amber-500/10 border border-amber-500/20 text-xs font-bold px-3 py-2 rounded-xl flex items-center gap-2 ${isOverdue ? 'text-red-600 dark:text-red-400 border-red-500/20 bg-red-500/10' : 'text-amber-600 dark:text-amber-400'}`}>
                          ...
                          {isOccupied && activeBooking && (
<div key="ap-div-a2qu94l" className={`border p-3.5 rounded-xl space-y-1.5 text-xs transition-colors ${
                              ...
                              {(() => {
                                ...
                                return (
<div key="ap-div-d6vbkj" className="space-y-1.5 pt-1.5 border-t border-dashed border-zinc-750">
                                    ...
                                    {unpaidDrinks > 0 && (
<div key="ap-div-0z7e4i" className="flex justify-between items-center text-[11px] text-amber-500 font-semibold">
                                    ...
                                    {overpaidAmount > 0 && (
<div key="ap-div-d7bi1k" className="mt-2 p-2 rounded-lg bg-amber-500/10 border border-amber-500/30 text-[10px] text-amber-600 dark:text-amber-400 font-medium space-y-1">
                          ...
                          {!isOccupied && (
<div key="ap-div-7uk8r" className={`p-3.5 rounded-xl text-xs space-y-2.5 my-1 border transition-colors shadow-sm ${
                              effectiveStatus === 'Available'
                                ...
                                <div className="space-y-1">
                                  ...
                                  <div className="flex flex-wrap gap-1">
                                    {room.amenities.map((amenity, idx) => (
                                      <span key={generateKey(room.id || room.roomNumber, idx, 'amenity')} className={`px-2 py-0.5 rounded-md text-[9px] font-bold border ${
                          ...
                          <div className={`pt-2 border-t flex flex-col gap-2 ${isDarkMode ? 'border-zinc-850/60' : 'border-slate-100'}`}>
                            {showCrossBranch || (room.branch && room.branch !== branch) ? (
                              ...
                              <>
                                {effectiveStatus === 'Available' && (
<div key="ap-div-w48p7" className="space-y-2">
                                    <button
                                      onClick={() => handleOpenBooking(room)}
                                ...
                                {effectiveStatus === 'Occupied' && (
<div key="ap-div-2f1m7u" className="grid grid-cols-3 gap-2">
                                    <button
                                      onClick={() => {
                                        if (activeBooking) {
                                          setEditingBookingTarget(activeBooking);
                                ...
                                {effectiveStatus === 'Cleaning' && (
<div key="ap-div-3i2krs" className="grid grid-cols-2 gap-2">
                                    <button
...
<div key="ap-div-2mdfhd" className="grid grid-cols-2 gap-2">
                                    <button
                                      onClick={() => handleUpdateRoomStatus(room, 'Available')}
                                      className={`py-2 px-3 rounded-xl text-xs font-bold transition-all cursor-pointer flex items-center justify-center gap-1.5 ${
                                        isDarkMode ? 'bg-emerald-500/10 text-emerald-400 hover:bg-emerald-500/20' : 'bg-emerald-50 text-emerald-600 hover:bg-emerald-100'
                  ...
                  {rooms.length === 0 && (
<div key="ap-div-c0yc6f" className={`col-span-1 md:col-span-2 py-12 text-center rounded-3xl border transition-colors ${
...
<motion.div
            ...
            >
              ...
              <form onSubmit={handleInlineReservationSubmit} className="space-y-5 max-h-[65vh] overflow-y-auto pr-2 scrollbar-thin">
                
                {/* Section 1: Guest Information (Name first, then Phone) */}
                <div className={`p-4 rounded-2xl border ${isDarkMode ? 'bg-zinc-900/60 border-zinc-800' : 'bg-slate-50/80 border-slate-200'} space-y-3`}>
                  ...
                  <div>
                    ...
                    >
                      ...
                      {rooms
                        ...
                        .map((r, idx) => (
                          <option key={generateKey(r.id || r.roomNumber, idx, 'res-room')} value={r.id}>
                  ...
                  <div className="relative pt-1">
                    ...
                    {showCalendarPopover && (
                      <div className="fixed inset-0 z-50 flex items-center justify-center pointer-events-auto p-4">
                        <div className="absolute inset-0 bg-black/60 backdrop-blur-sm" onClick={() => setShowCalendarPopover(false)} />
                        
                        <div className={`relative z-50 w-full max-w-sm rounded-3xl p-5 shadow-2xl border transition-all ${
                          isDarkMode 
                            ? 'bg-zinc-900 border-zinc-800 text-zinc-100 shadow-[0_20px_50px_rgba(0,0,0,0.7)]' 
                            : 'bg-white border-slate-200 text-slate-800 shadow-[0_20px_50px_rgba(0,0,0,0.15)]'
                        } space-y-4`}>
                          <div className="flex justify-between items-center pb-3 border-b border-zinc-100 dark:border-zinc-800">
                            <div className="text-[11px] font-semibold text-zinc-500 dark:text-zinc-400">
                              {!resRoomId ? (
                                <span className="text-amber-500 font-bold">⚠ Select a room above first</span>
                              ) : (
                                <span className="text-emerald-500 font-bold">✓ Live Availability Calendar</span>
                              )}
                            </div>
                            <button
                              type="button"
                              onClick={() => setShowCalendarPopover(false)}
                              className="px-3 py-1.5 text-xs font-bold text-white bg-blue-600 hover:bg-blue-700 rounded-xl shadow-md transition-colors cursor-pointer"
                            >
                              Done
                            </button>
                          </div>
                          ...
                          <div className="pt-2 text-center">
                            <span className="text-[10px] text-zinc-400 font-mono">
                              Click check-in date then check-out date to set stay duration.
                            </span>
                          </div>
                        </div>
                      </div>
                ...
                <div className={`p-4 rounded-2xl border ${isDarkMode ? 'bg-zinc-900/60 border-zinc-800' : 'bg-slate-50/80 border-slate-200'} space-y-3`}>
                  <div className="flex items-center gap-2 text-xs font-mono uppercase tracking-wider font-bold text-amber-600 dark:text-amber-400">
                    <span>3. Payment & Financials</span>
                  </div>
                  <div>
                    <label className="block text-xs font-mono uppercase tracking-wider text-slate-500 dark:text-zinc-400 mb-1.5 font-bold">
                      Payment Status *
                ...
                {resCheckIn && resCheckOut && (() => {
                  ...
                  return (
<div key="ap-div-fdt2ja" className="space-y-3 pt-2 border-t border-zinc-200 dark:border-zinc-850">
                      {days >= 10 && (
<div key="ap-div-mgi7sn" className="flex items-center justify-between">
                ...
                {resError && (
<div key="ap-div-jihw8o" className="p-3 bg-red-950/40 border border-red-900 text-xs text-red-400 rounded-xl font-mono">
                    ⚠ {resError}
                  </div>
                )}
              ...
              <div className="mt-8 border-t pt-8 dark:border-zinc-800">
                    ...
                    <tbody className="divide-y divide-zinc-100 dark:divide-zinc-800/60">
                      {(() => {
                        ...
                        if (todaysBookings.length === 0) {
                          return (
                            <tr>
                              <td colSpan={7} className="py-16 text-center px-6">
                        ...
                        return todaysBookings
                          .slice((todayBookingPage - 1) * todayBookingPageSize, todayBookingPage * todayBookingPageSize)
                          .map((b, idx) => {
                          const isArrival = b.isArrival;
                          return (
                            <tr key={generateKey(b.id, idx, 'todays-booking')} className="hover:bg-zinc-50 dark:hover:bg-zinc-800/40 transition-colors">
                              <td className="px-6 py-4 text-sm text-zinc-900 dark:text-zinc-100 font-bold border-b border-zinc-100 dark:border-zinc-800/60">
                                {b.guestName}
                              </td>
                              <td className="px-6 py-4 text-sm text-zinc-500 dark:text-zinc-400 border-b border-zinc-100 dark:border-zinc-800/60 font-mono">
                                <div className="flex items-center gap-1.5">
                                  <Phone className="w-3.5 h-3.5 text-blue-400 shrink-0" />
                                  <span className={b.guestContact || (b as any).guestPhone ? (isDarkMode ? 'text-zinc-200' : 'text-slate-800') : 'text-zinc-500'}>
                                    {b.guestContact || (b as any).guestPhone || '—'}
                                  </span>
                                </div>
                              </td>
                                ...
                                <div className="flex gap-2">
                                  {b.status === 'CheckedIn' ? (
                                    ...
                                    <button
                                      ...
                                      className="px-3 py-1.5 bg-indigo-600 hover:bg-indigo-500 text-white text-xs font-bold rounded-lg shadow-sm transition-colors cursor-pointer whitespace-nowrap"
                                    >
                                      Invoice / Receipt
                                    </button>
                                  ) : (
                ...
                <TablePagination
                  currentPage={todayBookingPage}
                  totalItems={(() => {
                    const userProfile = { ...currentUser, assignedBranch: currentUser.branch || 'Annex' };
                    const branchFiltered = bookings.map(b => ({ ...b, assignedBranch: b.branch })).filter(b => userProfile.role === 'Receptionist' ? b.assignedBranch === userProfile.assignedBranch : true);
                    const today = new Date(); today.setHours(0, 0, 0, 0); const tomorrow = new Date(today); tomorrow.setDate(tomorrow.getDate() + 1);
                    const todaysCheckIns = branchFiltered.filter(b => { const checkInDate = (b.checkInDate as any)?.seconds ? new Date((b.checkInDate as any).seconds * 1000) : new Date(b.checkInDate); return checkInDate >= today && checkInDate < tomorrow && !['checked_out', 'CheckedOut', 'Cancelled', 'No Show'].includes(b.status); });
                    const todaysCheckOuts = branchFiltered.filter(b => { if (b.status === 'CheckedOut' || b.status === 'checked_out') { if (!b.actualCheckOutDate) return false; const actualCheckOut = (b.actualCheckOutDate as any)?.seconds ? new Date((b.actualCheckOutDate as any).seconds * 1000) : new Date(b.actualCheckOutDate); return actualCheckOut >= today && actualCheckOut < tomorrow; } else { const checkOutDate = (b.checkOutDate as any)?.seconds ? new Date((b.checkOutDate as any).seconds * 1000) : new Date(b.checkOutDate); return checkOutDate >= today && checkOutDate < tomorrow; } });
                    const dateFiltered: Array<Booking & { isArrival: boolean }> = []; todaysCheckIns.forEach(b => dateFiltered.push({ ...b, isArrival: true })); todaysCheckOuts.forEach(b => { if (!dateFiltered.some(item => item.id === b.id)) dateFiltered.push({ ...b, isArrival: false }); });
                    const query = todayBookingSearch.toLowerCase().trim();
                    return dateFiltered.filter(b => !query || String(b.guestName || '').toLowerCase().includes(query) || String(b.roomNumber || '').toLowerCase().includes(query)).length;
                  })()}
                  pageSize={todayBookingPageSize}
                  onPageChange={setTodayBookingPage}
                  onPageSizeChange={setTodayBookingPageSize}
                  isDarkMode={isDarkMode}
                  itemName="today's bookings"
                />
              </div>
            </motion.div>
          )}
...
<motion.div
            ...
            >
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                ...
                <button
                  type="button"
                  onClick={handleManualSyncHistory}
                  disabled={isSyncingHistory}
                  title={`Last refreshed: ${lastHistorySyncTime}`}
                  className="px-3 py-2 bg-blue-600 hover:bg-blue-500 text-white font-bold text-xs rounded-xl transition-all flex items-center gap-1.5 shrink-0 cursor-pointer self-start sm:self-auto"
                >
                  <RefreshCw className={`w-3.5 h-3.5 ${isSyncingHistory ? 'animate-spin' : ''}`} />
                  {isSyncingHistory ? 'Syncing...' : 'Sync Registry'}
                  <span className="text-[10px] opacity-75 font-mono ml-0.5">({lastHistorySyncTime})</span>
                </button>
              </div>
...
<button key="ap-button-7x8tm"
              ...
              <div className={`overflow-x-auto border rounded-2xl ${theme.tableContainer}`}>
                <table className="w-full text-left border-collapse text-xs">
                  ...
                  <tbody className="divide-y divide-slate-100 dark:divide-zinc-850">
                    {paginatedHistoryBookings.length === 0 ? (
                      <tr>
                      ...
                      paginatedHistoryBookings.map((b, idx) => (
                        <tr key={generateKey(b.id, idx, 'filtered-booking')} className={`transition-colors ${theme.tableRowHover}`}>
                          ...
                          <td className="p-4 font-mono text-zinc-900 dark:text-zinc-50 tracking-wider font-bold text-xs">
                            ...
                            {b.guestEmail && <div className="text-[10px] text-zinc-500 dark:text-zinc-500 font-normal mt-0.5">{b.guestEmail}</div>}
                          </td>
                          <td className="p-4 font-mono text-xs">
                            <div className="flex items-center gap-1.5">
                              <Phone className="w-3.5 h-3.5 text-blue-400 shrink-0" />
                              <span className={b.guestContact || b.guestPhone ? (isDarkMode ? 'text-zinc-200' : 'text-slate-800') : 'text-zinc-500'}>
                                {b.guestContact || b.guestPhone || '—'}
                              </span>
                            </div>
                          </td>
                              ...
                              {(b.status === 'Pending' || b.status === 'Confirmed') && (
                                <>
                                  ...
                                  <button
                                    ...
                                    className="px-2 py-1 bg-zinc-700 hover:bg-zinc-600 text-white font-bold rounded-lg text-[10px] cursor-pointer flex items-center gap-1"
                                  >
...
<button key="ap-button-6ucqlv"
              ...
              <div className={`p-4 border rounded-2xl flex flex-col sm:flex-row items-center justify-between gap-3 text-xs ${isDarkMode ? 'border-zinc-800/60 bg-zinc-900/40 text-zinc-400' : 'border-slate-200 bg-slate-50 text-slate-600'}`}>
                <div className="flex items-center gap-3">
                  <span>
                    Showing <strong className={isDarkMode ? 'text-zinc-200' : 'text-slate-900'}>
                      {filteredHistoryBookings.length === 0 ? 0 : (safeHistoryPage - 1) * historyPageSize + 1}
                    </strong> to <strong className={isDarkMode ? 'text-zinc-200' : 'text-slate-900'}>
                      {Math.min(safeHistoryPage * historyPageSize, filteredHistoryBookings.length)}
                    </strong> of <strong className={isDarkMode ? 'text-zinc-200' : 'text-slate-900'}>{filteredHistoryBookings.length}</strong> bookings
                  </span>
                  <div className="flex items-center gap-1.5 ml-2">
                    ...
                    <select
                      ...
                      className={`px-2 py-1 rounded-lg text-xs font-medium focus:outline-none border cursor-pointer ${isDarkMode ? 'bg-zinc-800 border-zinc-700 text-zinc-200' : 'bg-white border-slate-300 text-slate-800'}`}
                    >
                ...
                <div className="flex items-center gap-1.5">
                  <button
                    ...
                    className={`px-2.5 py-1.5 rounded-lg border text-xs font-medium transition-colors ${
                      safeHistoryPage <= 1
                        ...
                        : isDarkMode ? 'hover:bg-zinc-800 border-zinc-700 text-zinc-200 cursor-pointer' : 'hover:bg-slate-200 border-slate-300 text-slate-800 cursor-pointer'
                    }`}
                  >
                    First
                  </button>
                  ...
                  <span className="px-2 font-mono text-xs">
                    Page <strong className={isDarkMode ? 'text-white' : 'text-slate-900'}>{safeHistoryPage}</strong> of <strong>{totalHistoryPages}</strong>
                  </span>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                    {drinks
                      ...
                      .map((drink, index) => {
                        ...
                        return (
<div key={generateKey(drink.id || drink.name, index, 'rec-drink')} className={`p-4 rounded-2xl border transition-all flex flex-col justify-between ${
                            isDarkMode 
                            ...
                            <div>
                              <div className="flex justify-between items-start gap-2 mb-2">
                                <span className={`font-bold text-sm ${isDarkMode ? 'text-white' : 'text-slate-900'} flex items-center gap-1.5`}>
                                  {index === 0 && totalSold > 0 && (
<span key="ap-span-astso" className="bg-amber-500/20 text-amber-500 text-[8px] font-extrabold px-1 py-0.5 rounded uppercase tracking-wider">👑 Best Seller</span>
                                  )}
                                  {drink.name}
                                </span>
                              ...
                              <div className="flex items-center justify-between mt-1 mb-3">
                                ...
                                {totalSold > 0 && (
<span key="ap-span-3sn2da" className="text-[10px] text-zinc-400 dark:text-zinc-500 font-mono">
...
<div className="fixed inset-0 bg-black/80 flex items-center justify-center p-4 z-50 backdrop-blur-sm" key="rd-modal-1">
            <motion.div
...
<div key="ap-div-rwcpsk" className="fixed inset-0 z-50 flex items-center justify-center pointer-events-auto">
...
<div key="ap-div-nei3mg" className="flex items-center justify-between pb-3.5 border-b border-zinc-200 dark:border-zinc-850">
                        <div className="pr-4">
                    ...
                    {bookingIsMonthly && (
<div key="ap-div-ohq5dv" className="space-y-1.5 pt-2 border-t border-dashed border-zinc-200 dark:border-zinc-850">
                        ...
                        >
                          {upcomingMonths.map((m, idx) => (
                            <option key={generateKey(m, idx, 'month')} value={m}>{m}</option>
                          ))}
                        </select>
                      </div>
                    )}
                    ...
                    <div className="space-y-1.5 pt-2 border-t border-zinc-200 dark:border-zinc-850">
                      ...
                      >
                        {Array.from(
                          ...
                          <option key={generateKey(num, idx, 'guest-opt')} value={num}>{num} {num === '1' ? 'Guest' : 'Guests'}</option>
                        ))}
                      </select>
                    </div>
                    ...
                    {checkInDate && checkOutDate && (() => {
                      ...
                      return (
                        <>
                          {days >= 10 && (
<div key="ap-div-apweh" className="flex items-center justify-between pt-2 border-t border-zinc-200 dark:border-zinc-850">
                              <label className={`text-xs font-bold ${isDarkMode ? 'text-zinc-100' : 'text-slate-800'}`}>Apply 5% Long-Stay Discount</label>
                              <input 
                                type="checkbox" 
                  ...
                  {(() => {
                    ...
                    if (checkInDateParsed && checkOutDateParsed && checkOutDateParsed > checkInDateParsed) {
                      ...
                      return (
<div key="ap-div-ed6zt" className="space-y-4">
                          <div className={`p-4 rounded-2xl border font-mono text-[11px] space-y-2 ${isDarkMode ? 'bg-zinc-950 border-zinc-850 text-zinc-300' : 'bg-slate-50 border-slate-200 text-slate-700'}`}>
                            ...
                            {bookingIsMonthly && bookingSelectedMonth && (
<div key="ap-div-3toae" className="flex justify-between text-purple-600 dark:text-purple-400 font-semibold">
                          ...
                          <div className={`p-4 rounded-2xl border ${isDarkMode ? 'bg-zinc-950 border-zinc-800' : 'bg-slate-50 border-slate-200'} space-y-3`}>
                            ...
                            {initialPaymentMode === 'partial' && (
<div key="ap-div-vikr0ku" className="pt-2">
                            ...
                            {initialPaymentMode === 'split' && (
<div key="ap-div-0tgsq" className="pt-2 space-y-2">
                                <div className="grid grid-cols-2 gap-2">
                                  <div>
                                    <label className={`block text-[10px] font-mono uppercase tracking-wider mb-1 ${isDarkMode ? 'text-zinc-400' : 'text-slate-500'}`}>
                                      Cash (GH₵)
...
<div key="ap-div-jrdqv" className="pt-2">
...
<div key="ap-div-lzri9c" className="fixed inset-0 bg-black/80 flex items-center justify-center p-4 z-50 backdrop-blur-sm">
            <motion.div
              initial={{ scale: 0.95, opacity: 0 }}
              animate={{ scale: 1, opacity: 1 }}
              exit={{ scale: 0.95, opacity: 0 }}
...
<div key="ap-div-nknmsp" className="flex justify-between text-amber-500">
                        ...
                        <>
                          ...
                          <div className="pl-2 space-y-0.5 text-[10px] text-zinc-500">
                            {checkoutUnpaidDrinks.map((sale, idx) => (
                              <div key={generateKey(sale.id, idx, 'sale')} className="flex justify-between">
...
<div key="ap-div-44j7axn" className="flex justify-between text-[11px] text-amber-500 font-bold">
                          <span>Late Check-Out Fee</span>
                  ...
                  {(() => {
                    ...
                    return (
<div key="ap-div-or3wfj" className="space-y-3 mb-4">
                        {hasPreviousDiscount && (
<div key="ap-div-34ebz" className={`p-3 rounded-2xl border text-xs flex items-center justify-between ${isDarkMode ? 'bg-blue-950/30 border-blue-800/50 text-blue-300' : 'bg-blue-50 border-blue-200 text-blue-800'}`}>
      ...
      <AnimatePresence>
        {showFutureStayModal && (
<div key="ap-div-zsfxzu" className="fixed inset-0 bg-black/80 flex items-center justify-center p-4 z-50 backdrop-blur-sm">
            <motion.div
              initial={{ scale: 0.95, opacity: 0 }}
              animate={{ scale: 1, opacity: 1 }}
              exit={{ scale: 0.95, opacity: 0 }}
              ...
              <div className="flex-1 overflow-y-auto p-6 pt-2 scrollbar-thin scroll-smooth">
                ...
                <div>
                  ...
                  >
                    ...
                    {rooms.map((rm, idx) => (
                      <option key={generateKey(rm.id || rm.roomNumber, idx, 'fut-rm')} value={rm.id}>
...
<div key="ap-div-2flxfo" className="absolute bottom-full left-0 mb-2 z-50 whitespace-nowrap bg-rose-950 text-rose-200 text-[10px] font-bold px-3 py-1.5 rounded-xl shadow-2xl border border-rose-900 animate-bounce">
                          ⚠️ Please select a room first.
                        </div>
                      )}

                      {activePopover === 'checkin' && futureRoomId && (
<div key="ap-div-i80ll" className={`absolute top-full left-0 mt-2 z-50 w-[300px] shadow-2xl rounded-2xl p-1 border ${isDarkMode ? 'bg-zinc-950 border-zinc-800 text-white' : 'bg-white border-slate-200 text-slate-800'}`}>
...
<div key="ap-div-f0jode" className="absolute bottom-full right-0 mb-2 z-50 whitespace-nowrap bg-rose-950 text-rose-200 text-[10px] font-bold px-3 py-1.5 rounded-xl shadow-2xl border border-rose-900 animate-bounce">
                          ⚠️ Please select a room first.
                        </div>
                      )}

                      {activePopover === 'checkout' && futureRoomId && (
<div key="ap-div-n3ua12" className={`absolute top-full right-0 mt-2 z-50 w-[300px] shadow-2xl rounded-2xl p-1 border ${isDarkMode ? 'bg-zinc-950 border-zinc-800 text-white' : 'bg-white border-slate-200 text-slate-800'}`}>
                          <div className="flex justify-end p-1">
                            <button
                              type="button"
                              onClick={() => setActivePopover(null)}
                ...
                {(() => {
                  ...
                  if (!r) return null;
                  return (
<div key="ap-div-uvir5" className={`p-4 rounded-2xl border ${isDarkMode ? 'bg-zinc-950 border-zinc-800' : 'bg-slate-50 border-slate-200'} space-y-4`}>
                      {/* Occasion Booking Mode (Only for Apartments) */}
                      {(r.roomType === '2 Bedroom Apartment' || r.roomType === '3 Bedroom Apartment') && (
<div key="ap-div-8r77ty" className="flex items-center justify-between pb-3.5 border-b border-zinc-200 dark:border-zinc-850">
                          <div className="pr-4">
                            <span className={`text-xs font-bold block ${isDarkMode ? 'text-zinc-100' : 'text-slate-800'}`}>Occasion Booking Mode</span>
                      ...
                      {/* Specify Month Dropdown (Shown only if Monthly Premium Package is checked) */}
                      {futureIsMonthly && (
<div key="ap-div-kxqc5b" className="space-y-1.5 pt-2 border-t border-dashed border-zinc-200 dark:border-zinc-850">
                          <label className={`block text-[10px] font-mono uppercase tracking-wider font-bold ${isDarkMode ? 'text-zinc-400' : 'text-slate-500'}`}>
                            Specify Booking Month *
                          </label>
                          <select
                      ...
                      <div className="space-y-1.5 pt-2 border-t border-zinc-200 dark:border-zinc-850">
                        ...
                        >
                          {Array.from(
                            ...
                            <option key={generateKey(num, idx, 'fut-guest-opt')} value={num}>{num} {num === '1' ? 'Guest' : 'Guests'}</option>
                          ))}
                        </select>
                      </div>
                    </div>
                ...
                {(() => {
                  ...
                  return (
<div key="ap-div-ohegm6" className="space-y-3">
                      {/* Discount Controls */}
                      {n >= 10 && (
<div key="ap-div-xh1o49" className="flex items-center justify-between pt-2 border-t border-zinc-200 dark:border-zinc-850">
                          <label className={`text-xs font-bold ${isDarkMode ? 'text-zinc-100' : 'text-slate-800'}`}>Apply 5% Long-Stay Discount</label>
                          <input 
                            type="checkbox" 
                      ...
                      {futureIsMonthly && futureSelectedMonth && (
<div key="ap-div-62hah5" className="flex justify-between text-purple-600 dark:text-purple-400 font-semibold">
                          <span>Package Month:</span>
                ...
                {(() => {
                  ...
                  return (
<div key="ap-div-gigb3p" className="space-y-3 pt-1">
                      <div>
                        <label className={`block text-[10px] font-mono uppercase tracking-wider mb-1 font-bold ${isDarkMode ? 'text-zinc-400' : 'text-slate-500'}`}>
                      ...
                      {(resPayment as string) === 'Split' && (
<div key="ap-div-5uprwv" className="p-3 bg-blue-50/50 dark:bg-blue-950/30 rounded-xl border border-blue-200 dark:border-blue-800/50 space-y-2">
                          <div className="grid grid-cols-2 gap-3">
                            <div>
                              <label className="block text-[10px] font-mono uppercase text-slate-500 dark:text-zinc-400 font-bold mb-1">
                                Cash Amount (GH₵)
                      ...
                      {resPayment === 'Partial' && (
<div key="ap-div-uo9yt" className="space-y-1.5 p-3 rounded-xl border bg-amber-50/50 dark:bg-amber-950/20 border-amber-200 dark:border-amber-800/40">
                ...
                {futureStayError && (
<div key="ap-div-5pszw" className="p-3 bg-red-950/40 border border-red-900 text-xs text-red-400 rounded-xl font-mono">
                    ⚠ {futureStayError}
                  </div>
                )}
      ...
      <AnimatePresence>
        {showArrivalModal && arrivalBooking && (() => {
          ...
          return (
<div key="ap-div-p84ut" className="fixed inset-0 bg-black/80 flex items-center justify-center p-4 z-50 backdrop-blur-sm">
              <motion.div
                initial={{ scale: 0.95, opacity: 0 }}
                animate={{ scale: 1, opacity: 1 }}
                exit={{ scale: 0.95, opacity: 0 }}
      ...
      <AnimatePresence>
        {showPrintInvoiceModal && invoiceBooking && (() => {
          ...
          const isFuture = invoiceBooking.status === 'Pending' || invoiceBooking.status === 'Confirmed' || invoiceBooking.bookingType === 'future' || (invoiceBooking as any).isFutureBooking;
          const isReceipt = balanceDue <= 0;
          let documentLabel = isReceipt ? 'Receipt' : 'Invoice';
          let displayStatusBadge = balanceDue > 0 ? 'PENDING SETTLEMENT' : (invoiceType === 'CheckOut' ? 'SETTLED' : 'ISSUED');
          
          if (isFuture) {
            if (paymentsMade > 0) {
              documentLabel = 'Deposit Receipt';
              displayStatusBadge = 'DEPOSIT SECURED';
            } else {
              documentLabel = 'Reservation Invoice';
              displayStatusBadge = 'UNPAID RESERVATION';
            }
          }
          ...
          return (
<div key="ap-div-zb9vf" className="fixed inset-0 bg-black/80 flex items-center justify-center p-4 z-50 backdrop-blur-sm">
              <style>{`
                @media print {
                  @page {
                    size: ${invoicePrintPaperSize === 'thermal' ? '80mm auto' : 'A4 portrait'};
                    margin: ${invoicePrintPaperSize === 'thermal' ? '0' : '15mm 12mm 15mm 12mm'};
                  }
                  body {
                  ...
                  .printable-invoice-modal {
                    ...
                    left: 0 !important;
                    top: 0 !important;
                    width: ${invoicePrintPaperSize === 'thermal' ? '76mm !important' : '100% !important'};
                    padding: ${invoicePrintPaperSize === 'thermal' ? '8px !important' : '0 !important'};
                    background: white !important;
                    color: black !important;
                    box-shadow: none !important;
                    font-size: ${invoicePrintPaperSize === 'thermal' ? '11px !important' : '13px !important'};
                  }
                  ...
                  ` : `
                    .printable-invoice-modal .invoice-header-row {
                      display: flex !important;
                      flex-direction: column !important;
                    ...
                    .printable-invoice-modal .invoice-header-right {
                      ...
                      <div className="space-y-2 text-xs">
                        ...
                        <label className="flex items-center gap-2.5 cursor-pointer">
                          <input
                            ...
                            className="rounded text-blue-600 focus:ring-blue-500 w-4 h-4"
                          />
                          <span className="font-semibold text-zinc-700 dark:text-zinc-300">Show Operator Name</span>
                        </label>
                  ...
                  <div className="lg:col-span-3 space-y-4">
                    ...
                    <div className={`p-6 rounded-2xl border bg-white text-zinc-900 shadow-xl overflow-y-auto ${
                      ...
                      <div 
                        ...
                        className={`printable-invoice-modal space-y-6 print:shadow-none ${
                          invoicePrintPaperSize === 'thermal' 
                            ? 'thermal-sheet print:w-[76mm] print:p-2 print:text-xs print:space-y-3' 
                            : 'a4-sheet print:w-full print:p-0 print:text-sm print:space-y-6'
                        }`}
                      >
                        
                        {/* Letterhead Logo / Branding Header - Option A: Clean Side-by-Side Brand Banner */}
                        {invoicePrintShowBranding && (
                          <div className={`invoice-header-row border-b-2 border-slate-900/10 dark:border-zinc-800 pb-4 print:pb-2 print:mb-2 print:border-b w-full ${
                            invoicePrintPaperSize === 'thermal'
                              ? 'flex flex-col gap-3 text-left'
                              : 'flex flex-row justify-between items-center gap-4'
                          }`}>
                            
                            {/* Left Side: Brand Artwork + Branch & Document Metadata */}
                            <div className="flex items-center gap-3">
                              <div className="relative p-1.5 bg-white dark:bg-zinc-900 rounded-xl shadow-sm border border-slate-200/80 dark:border-zinc-800 shrink-0">
                                <NabsLodgeLogo size={invoicePrintPaperSize === 'thermal' ? 'md' : 'xl'} className="border-0 shadow-none !border-none" />
                              </div>
                              <div className="border-l-2 border-blue-600 dark:border-blue-500 pl-3 space-y-0.5">
                                <p className="text-sm sm:text-base font-extrabold text-slate-900 dark:text-white uppercase tracking-wider print:text-sm" style={{ margin: 0, lineHeight: 1.1 }}>
                                  Nabslodge {branch}
                                </p>
                                <p className="text-[10px] font-semibold text-blue-600 dark:text-blue-400 tracking-tight print:text-[10px]">
                                  Official {isFuture ? 'Stay Reservation' : (invoiceType === 'CheckIn' ? 'Check-In Booking' : 'Check-Out Settlement')} {documentLabel}
                                </p>
                              </div>
                            </div>

                            {/* Right Side: Billing Metadata & Official Status Badge */}
                            <div className={`invoice-header-right font-mono text-xs space-y-1 print:text-[10px] ${
                              invoicePrintPaperSize === 'thermal'
                                ? 'text-left pt-2 border-t border-dashed border-zinc-200 dark:border-zinc-800 w-full'
                                : 'text-right shrink-0'
                            }`}>
                              <div className="text-[10px] font-bold uppercase tracking-wider text-slate-400 dark:text-zinc-500 mb-0.5 print:text-[9px]">
                                {documentLabel} REFERENCE
                              </div>
                              <div className="text-sm font-black text-slate-900 dark:text-white print:text-xs">
                                <span className="text-slate-400 font-normal">No: </span>
                                {invoiceNum}
                              </div>
                              <div className="text-slate-600 dark:text-zinc-400 text-[11px] print:text-[9px]">
                                <span className="text-slate-400">Issue Date: </span>{dateOfIssue}
                              </div>
                              <div className="text-slate-500 dark:text-zinc-400 text-[10px] print:text-[8px]">
                                <span className="text-slate-400">Time: </span>{currentTime}
                              </div>
                              <div className="pt-1">
                                <span className={`inline-block px-2.5 py-0.5 rounded-full text-[10px] font-extrabold uppercase tracking-wider border ${
                                  balanceDue > 0 
                                    ...
                                    : (invoiceType === 'CheckOut' ? 'bg-emerald-500/10 text-emerald-600 border-emerald-500/30' : 'bg-blue-500/10 text-blue-600 border-blue-500/30')
                                }`}>
                                  {displayStatusBadge}
                                </span>
                              </div>
                        ...
                        {isFuture && (
                          <div className={`p-2.5 text-center rounded-lg border text-[10px] font-mono font-bold leading-tight uppercase ${
                            paymentsMade > 0 
                              ? 'bg-emerald-50/80 border-emerald-200 text-emerald-800' 
                              : 'bg-zinc-50 border-zinc-200 text-zinc-600'
                          }`}>
                            {paymentsMade > 0 
                              ? '⚡ Stay Secured — Present this Receipt upon Check-In' 
                              : '⏳ Reservation Pending — Balance Settlement Required'}
                          </div>
                        )}

                        {/* Guest and Lodging details */}
                        <div className={`grid gap-6 py-2 print:flex print:flex-col print:gap-2 print:py-1 ${
                          invoicePrintPaperSize === 'thermal' ? 'grid-cols-1' : 'grid-cols-1 md:grid-cols-2'
                        }`}>
                          <div>
                            ...
                            <div className="space-y-1 text-sm print:text-xs print:space-y-0.5">
                              ...
                              {invoicePrintShowStaff && (
                                <div key="ap-div-2d9jgd" className="text-zinc-400 text-xs print:text-[10px]">Processed by: {invoiceBooking.receptionistName}</div>
                              )}
                            </div>
                          </div>
                          <div>
                          ...
                          <div className="border border-zinc-200 rounded-lg overflow-hidden print:border-zinc-300">
                            <table className="w-full text-left text-xs font-mono print:text-[10px]">
                              ...
                              <tbody className="divide-y divide-zinc-200 print:divide-zinc-300">
                                ...
                                {hasInvDiscount && (
<tr key="ap-tr-iseeu" className="bg-blue-50/60 print:bg-slate-50">
                                    ...
                                    <td className="p-3 print:p-1.5">
                                      ...
                                      <ul className="text-[9px] text-zinc-400 list-disc ml-4 print:text-[8px]">
                                        {unpaidDrinksList.map((s, idx) => (
                                          <li key={generateKey(s.id || s.drinkName || idx, idx, 'unpaid-drink')}>{s.drinkName} (x{s.quantity})</li>
                                        ))}
                                      </ul>
                                    </td>
                        ...
                        <div className="flex justify-end pt-2 print:pt-1 print:w-full">
                          <div className="w-full sm:w-80 space-y-2.5 font-mono text-xs print:w-full print:space-y-1 print:text-[10px]">
                            {hasInvDiscount && (
<div key="ap-div-y6sxo8" className="p-2.5 rounded-xl bg-blue-50/80 border border-blue-200 space-y-1 text-xs print:p-1.5">
                            ...
                            <div className="pt-1 space-y-1 text-emerald-600 font-semibold print:text-black">
                              {hasLateFee && Number(invoiceBooking?.priorAmountPaid || 0) > 0 ? (
                                ...
                                <div className="flex justify-between">
                                  <span>{isFuture ? (paymentsMade > 0 ? 'Deposit Secured (Paid):' : 'Deposit Received:') : 'Payments Made:'}</span>
                                  <span>- GH₵{paymentsMade.toFixed(2)}</span>
                                </div>
                              )}
                            </div>
                            ...
                            <div className="border-t border-double border-zinc-300 pt-2.5 flex justify-between font-black text-base text-blue-600 print:pt-1 print:border-t-dashed print:border-zinc-300 print:text-black print:text-xs">
                              <span>{isFuture ? 'Balance Due at Check-In:' : (isReceipt ? 'Balance Remaining:' : 'Total Balance Due:')}</span>
                              <span className={isFuture && balanceDue > 0 ? 'text-amber-600 dark:text-amber-500 font-extrabold' : ''}>GH₵{balanceDue.toFixed(2)}</span>
                            </div>
                          </div>
                        </div>
...
<div key="ap-div-sf6345" className="grid grid-cols-2 gap-6 pt-8 border-t border-dashed border-zinc-200 mt-6 print:pt-4 print:mt-4">
                        ...
                        <div className="text-center pt-8 border-t border-zinc-100 space-y-1 print:pt-4 print:border-t-dashed print:border-zinc-300">
                          <p className="text-[10px] text-zinc-500 italic print:text-[8px]">
                            "{invoicePrintMemo ? invoicePrintMemo
                              .replace(/Note:\s*ALL CHARGES ARE FLAT\.?/gi, "")
                              .replace(/All charges are flat\.?/gi, "")
                              .replace(/Tax exempted\.?/gi, "")
                              .replace(/Tax exempt\.?/gi, "")
                              .trim() : ""}"
                          </p>
...
<div className="fixed inset-0 bg-black/80 flex items-center justify-center p-4 z-50 backdrop-blur-md" key="rd-modal-2">
            <motion.div
              initial={{ scale: 0.95, opacity: 0 }}
              animate={{ scale: 1, opacity: 1 }}
              exit={{ scale: 0.95, opacity: 0 }}
      ...
      <div className="fixed bottom-6 right-6 z-40 flex flex-col items-end gap-3">
        <AnimatePresence>
          {isFabOpen && (
<motion.div key="ap-motion-div-1l8q6b"
...
<div key="ap-div-nmnjmm" className="fixed inset-0 bg-black/80 flex items-center justify-center p-4 z-50 backdrop-blur-sm">
            <motion.div
              initial={{ scale: 0.95, opacity: 0 }}
              animate={{ scale: 1, opacity: 1 }}
              exit={{ scale: 0.95, opacity: 0 }}
              ...
              <form onSubmit={(e) => { handleInlineReservationSubmit(e); if (!resError) setShowQuickCheckInModal(false); }} className="space-y-4 text-xs">
                <div>
                  ...
                  >
                    ...
                    {dynamicRooms.filter(r => getRoomEffectiveStatus(r) === 'Available').map((r, idx) => (
                      <option key={generateKey(r.id || r.roomNumber, idx, 'dyn-rm')} value={r.id}>Room {r.roomNumber} - {r.roomType} (GH₵{r.price}/night)</option>
                    ))}
                  </select>
                </div>
...
<div key="ap-div-mjzmp" className="fixed inset-0 z-50 flex items-center justify-center pointer-events-auto">
                ...
                <div>
                  ...
                  {(resPayment as string) === 'Split' && (
<div key="ap-div-6w1sgp" className="mt-3 p-3 bg-blue-50/50 dark:bg-blue-950/30 rounded-xl border border-blue-200 dark:border-blue-800/50 space-y-2">
                      <div className="grid grid-cols-2 gap-3">
                        <div>
                          <label className="block text-[10px] font-mono uppercase text-slate-500 dark:text-zinc-400 font-bold mb-1">
                            Cash Amount (GH₵)
                  ...
                  {resPayment !== 'Unpaid' && (resPayment as string) !== 'Split' && (
<div key="ap-div-frpoan" className="mt-3">
                ...
                {resError && (
<div key="ap-div-70b4wk" className="p-3 bg-red-950/40 border border-red-900 text-xs text-red-400 rounded-xl font-mono">
                    ⚠ {resError}
                  </div>
                )}
      ...
      <AnimatePresence>
        {showGuestModal && (
<div key="ap-div-gut1ye" className="fixed inset-0 bg-black/80 flex items-center justify-center p-4 z-50 backdrop-blur-sm">
            <motion.div
              initial={{ scale: 0.95, opacity: 0 }}
              animate={{ scale: 1, opacity: 1 }}
              exit={{ scale: 0.95, opacity: 0 }}
            ...
            >
              ...
              {(() => {
                ...
                return (
<div key="ap-div-9sok0b" className="space-y-4">
                    ...
                    <div>
                      ...
                      <div className="space-y-2">
                        {allBookings.map((b, idx) => (
                          <div key={generateKey(b.id, idx, 'multi-lodge-history')} className={`p-3 rounded-xl border flex justify-between items-center ${isDarkMode ? 'bg-zinc-950/50 border-zinc-800' : 'bg-white border-slate-100 shadow-xs'}`}>
                            <div>
...
<div key="ap-div-ts7dw" className="fixed inset-0 bg-black/80 flex items-center justify-center p-4 z-50 backdrop-blur-sm">
            <motion.div
              initial={{ scale: 0.95, opacity: 0 }}
              animate={{ scale: 1, opacity: 1 }}
              exit={{ scale: 0.95, opacity: 0 }}
            ...
            >
              ...
              {rooms.filter(r => r.status === 'Maintenance').length === 0 ? (
                ...
                <div className="space-y-4">
                  {rooms.filter(r => r.status === 'Maintenance').map((room, idx) => (
                    <div key={generateKey(room.id || room.roomNumber, idx, 'maint-room')} className={`border rounded-2xl p-4 flex flex-col md:flex-row md:items-center justify-between gap-4 ${isDarkMode ? 'bg-zinc-950 border-zinc-850' : 'bg-white border-slate-200 shadow-sm'}`}>
      ...
      <AnimatePresence>
        {showHistoryPinModal && (
<div key="ap-div-irsjto" className="fixed inset-0 bg-black/80 flex items-center justify-center p-4 z-50 backdrop-blur-sm">
            <motion.div
              initial={{ scale: 0.95, opacity: 0 }}
              animate={{ scale: 1, opacity: 1 }}
              exit={{ scale: 0.95, opacity: 0 }}
            ...
            >
              ...
              <div className="flex flex-col items-center text-center">
                ...
                {pinError && (
<p key="ap-p-3vca7l" className="text-red-500 text-xs font-bold mb-4">{pinError}</p>
                )}
      ...
      <AnimatePresence>
        {showSignOutModal && (
<div key="ap-div-8gvxj" className="fixed inset-0 bg-black/80 flex items-center justify-center p-4 z-50 backdrop-blur-sm">
            <motion.div
              initial={{ scale: 0.95, opacity: 0 }}
              animate={{ scale: 1, opacity: 1 }}
              exit={{ scale: 0.95, opacity: 0 }}
      ...
      <AnimatePresence>
        {showHandoverModal && (
<div className="fixed inset-0 z-50 flex items-center justify-center p-4" key="rd-modal-3">
            <motion.div
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
      ...
      <AnimatePresence>
        {showWalkInModal && (
<div key="ap-div-3jtc5vh" className="fixed inset-0 bg-black/80 flex items-center justify-center p-4 z-50 backdrop-blur-sm">
            <motion.div
              initial={{ scale: 0.95, opacity: 0 }}
              animate={{ scale: 1, opacity: 1 }}
              exit={{ scale: 0.95, opacity: 0 }}
            ...
            >
              ...
              <p className={`text-xs mb-6 ${isDarkMode ? 'text-zinc-400' : 'text-slate-500'}`}>
                Fast-track billing console for non-resident activities.
              </p>
              ...
              <form onSubmit={handleSaveWalkIn} className="space-y-6 text-xs">
                    ...
                    >
                      {activityCatalog.length > 0 ? (
                        activityCatalog.map((item, idx) => (
                          <option key={generateKey(item.id || item.name, idx, 'act-cat-opt')} value={item.name}>
                ...
                {walkInError && (
<div key="ap-div-0sis47" className="p-3 bg-red-950/40 border border-red-900 text-xs text-red-400 rounded-xl font-mono whitespace-pre-line">
                    ⚠ {walkInError}
                  </div>
                )}
...
<EditBookingModal key="edit-booking-modal"
...
<TransferRoomModal key="transfer-room-modal"
      ...
      <AnimatePresence>
        {showDrinkOrderModal && (
<div key="ap-div-s35j7w" className="fixed inset-0 z-50 flex items-center justify-center p-4">
            <motion.div
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              exit={{ opacity: 0 }}
              ...
              <form onSubmit={handleProcessDrinkSale} className="space-y-5 text-xs">
                ...
                <div className="space-y-2">
                  ...
                  <div className={`flex p-1 rounded-xl border ${isDarkMode ? 'bg-zinc-950/50 border-zinc-800' : 'bg-slate-100 border-slate-200'}`}>
                    {(['walkin', 'room', 'staff'] as const).map((type, idx) => (
                      <button
                        key={generateKey(type, idx, 'item')}
                        type="button"
                        onClick={() => {
...
<div key="ap-div-4op6d" className="space-y-3">
                      <div className="space-y-1.5">
                        <label className={`block text-[10px] font-mono uppercase tracking-wider font-bold ${isDarkMode ? 'text-zinc-400' : 'text-slate-500'}`}>
                          Guest Name (Optional)
                        </label>
...
<div key="ap-div-c0cjik" className="space-y-3">
                      <div className="space-y-1.5">
                        <label className={`block text-[10px] font-mono uppercase tracking-wider font-bold ${isDarkMode ? 'text-zinc-400' : 'text-slate-500'}`}>
                        ...
                        >
                          ...
                          {branchBookings.filter(b => b.status === 'CheckedIn').map((b, idx) => (
                            <option key={generateKey(b.id, idx, 'b-opt')} value={b.id}>
                      ...
                      {!drinkBookingId && (
<div key="ap-div-nznfef" className="space-y-1.5">
                          <label className={`block text-[10px] font-mono uppercase tracking-wider font-bold ${isDarkMode ? 'text-zinc-400' : 'text-slate-500'}`}>
                          ...
                          >
                            ...
                            {dynamicRooms.filter(r => getRoomEffectiveStatus(r) === 'Occupied').map((r, idx) => {
                              const activeB = getActiveBookingForRoom(r, bookings, [...rooms, ...otherBranchRooms]);
                              return (
<option key={generateKey(r.roomNumber || r.id, idx, 'r-num-opt')} value={r.roomNumber}>
...
<div key="ap-div-khl8e" className="space-y-3">
                      <div className="space-y-1.5">
                        <label className={`block text-[10px] font-mono uppercase tracking-wider font-bold ${isDarkMode ? 'text-zinc-400' : 'text-slate-500'}`}>
                          Select Staff Member *
                        </label>
...
<div key="ap-div-kdbdi9" className={`absolute top-full left-0 right-0 mt-2 z-[100] rounded-xl border shadow-2xl max-h-60 overflow-y-auto animate-in slide-in-from-top-2 duration-200 ${
                                    isDarkMode ? 'bg-zinc-900 border-zinc-800' : 'bg-white border-slate-200'
                                  }`}>
                                    ...
                                    {staff
                                      ...
                                      .map((s, idx) => (
                                        <button
                                          key={generateKey(s.id, idx, 'staff-btn')}
                                          type="button"
                                          onClick={() => {
...
<div key="ap-div-h8n57" className="px-4 py-8 text-center">
                ...
                <div className={`space-y-3 p-4 rounded-2xl border ${isDarkMode ? 'bg-zinc-900/40 border-zinc-700' : 'bg-white border-zinc-200'}`}>
                  <div className="flex gap-2 items-end">
                    <div className="flex-1 space-y-1.5">
                      ...
                      >
                        ...
                        {drinks.filter(d => d.inStock !== false).map((d, idx) => (
                          <option key={generateKey(d.id, idx, 'drink-opt')} value={d.id}>
                  ...
                  {drinkCart.length > 0 && (
<div key="ap-div-6fw7b" className={`space-y-2 mt-3 pt-3 border-t ${isDarkMode ? 'border-zinc-800' : 'border-zinc-200'}`}>
                      {drinkCart.map((item, idx) => (
                        <div key={generateKey(item.drinkId || item.drinkName || idx, idx, 'cart-item')} className={`flex justify-between items-center p-2 rounded-lg ${isDarkMode ? 'bg-zinc-900/50' : 'bg-zinc-50'}`}>
...
<button key="ap-button-kxxwv"
              ...
              {(drinkCustomerType === 'room' || drinkCustomerType === 'staff') && (
<div key="ap-div-ggpcq" className="mt-2">
                  <button
                    type="button"
                    onClick={() => {
                  ...
                  {drinkPaymentMethod === 'Split (Cash + Momo)' && (
<div key="ap-div-l78qsd" className={`grid grid-cols-2 gap-3 mt-3 p-3 rounded-xl border ${isDarkMode ? 'bg-indigo-500/5 border-indigo-500/20' : 'bg-indigo-50 border-indigo-100'}`}>
                  ...
                  {drinkPaymentMethod === 'Split (Paid & Unpaid)' && (
<div key="ap-div-6o2h1s" className={`mt-3 p-3.5 rounded-xl border space-y-3 ${isDarkMode ? 'bg-purple-500/5 border-purple-500/20' : 'bg-purple-50 border-purple-100'}`}>
...
<div 
            className="fixed inset-0 bg-black/80 flex items-center justify-center p-4 z-[60] backdrop-blur-sm" key="rd-modal-4"
              ...
              <form onSubmit={handleEditDrinkSaleSubmit} className="space-y-4">
                <div className="space-y-1.5">
                  ...
                  >
                    ...
                    {drinks.map((d, idx) => (
                      <option key={generateKey(d.id, idx, 'edit-drink')} value={d.id}>
                        {d.name} (GH₵ {d.price.toFixed(2)})
                      </option>
                    ))}
                  </select>
                ...
                <div className="space-y-1.5">
                  ...
                  >
                    ...
                    {dynamicRooms.filter(r => getRoomEffectiveStatus(r) === 'Occupied').map((r, idx) => {
                      const activeB = getActiveBookingForRoom(r, bookings, [...rooms, ...otherBranchRooms]);
                      return (
<option key={generateKey(r.roomNumber || r.id, idx, 'edit-rm')} value={r.roomNumber}>
                          Room {r.roomNumber} ({activeB?.guestName || r.guestName || 'Occupied'})
                        </option>
                      );
                    })}
                ...
                <div className="space-y-1.5">
                  ...
                  {editDrinkSalePaymentMethod === 'Split (Cash + Momo)' && (
<div key="ap-div-hypn1e" className={`grid grid-cols-2 gap-3 mt-3 p-3 rounded-xl border ${isDarkMode ? 'bg-indigo-500/5 border-indigo-500/20' : 'bg-indigo-50 border-indigo-100'}`}>
                      <div className="flex flex-col justify-end">
                        <label className={`block text-[10px] font-mono mb-1 leading-tight ${isDarkMode ? 'text-zinc-400' : 'text-slate-500'}`}>Cash Amount (GH₵)</label>
                        <input
                          type="number"
                  ...
                  {editDrinkSalePaymentMethod === 'Split (Paid & Unpaid)' && (
<div key="ap-div-n7baej" className={`mt-3 p-3.5 rounded-xl border space-y-3 ${isDarkMode ? 'bg-purple-500/5 border-purple-500/20' : 'bg-purple-50 border-purple-100'}`}>
                      <div className="flex items-center justify-between gap-2 pb-1 border-b border-purple-200/40 dark:border-purple-800/30">
                        <span className={`text-[11px] font-bold ${isDarkMode ? 'text-purple-300' : 'text-purple-900'}`}>
                          Payment Method for Paid Portion:
                        </span>
...
<div 
            className="fixed inset-0 bg-black/90 flex items-center justify-center p-4 z-[70] backdrop-blur-md overflow-y-auto" key="rd-modal-5"
      ...
      <AnimatePresence>
        {showArrivalsModal && (
<div 
            className="fixed inset-0 bg-black/80 flex items-center justify-center p-4 z-[75] backdrop-blur-sm overflow-y-auto" key="rd-modal-6"
            ...
            >
              ...
              <div className="p-6 space-y-3.5 max-h-[65vh] overflow-y-auto">
                {todaysArrivals.length === 0 ? (
                  ...
                  todaysArrivals.map((b, idx) => {
                    const r = rooms.find(rm => rm.id === b.roomId || String(rm.roomNumber) === String(b.roomNumber));
                    const phone = b.guestContact || b.guestPhone || '';
                    
                    const total = Number(b.totalPrice ?? b.totalRate ?? b.normalPrice ?? 0);
                    const deposit = Number(b.deposit ?? b.amountPaid ?? b.amountReceived ?? b.priorAmountPaid ?? (b.paymentStatus === 'Paid' ? total : 0));
                    const rawBalance = b.balance_due !== undefined ? Number(b.balance_due) : (b.pending_payment !== undefined ? Number(b.pending_payment) : (total - deposit));
                    const balance = Math.max(0, rawBalance);
                    const isFullyPaid = balance <= 0 || b.paymentStatus === 'Paid';

                    return (
<div
                        key={generateKey(b.id || idx, idx, 'arrival-card')}
                      ...
                      >
                        <div className="flex flex-col md:flex-row md:items-center justify-between gap-2 border-b border-zinc-200 dark:border-zinc-800/60 pb-3">
                          <div className="space-y-1">
                        ...
                        <div className="grid grid-cols-3 gap-2 bg-zinc-200/50 dark:bg-zinc-950/60 p-2.5 rounded-xl text-xs">
                          <div>
                            <span className="text-[10px] text-zinc-400 uppercase tracking-wider block font-semibold">Total Cost</span>
                            <span className="font-bold text-zinc-700 dark:text-zinc-200">
                              GH₵ {total.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                            </span>
                          </div>
                          <div>
                            <span className="text-[10px] text-zinc-400 uppercase tracking-wider block font-semibold">Deposit Paid</span>
                            <span className="font-bold text-emerald-600 dark:text-emerald-400">
                              GH₵ {deposit.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                            </span>
                          </div>
                          <div>
                            <span className="text-[10px] text-zinc-400 uppercase tracking-wider block font-semibold">Balance Due</span>
                            {isFullyPaid ? (
                              ...
                              <span className="font-bold text-amber-600 dark:text-amber-400">
                                GH₵ {balance.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                              </span>
                            )}
                          </div>
                        </div>
