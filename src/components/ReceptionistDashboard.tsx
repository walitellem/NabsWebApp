...
import { 
  User, Room, Booking, AuditLog, Branch, RoomStatus, BookingStatus, PaymentStatus, Role, WalkInActivityInput, HandoverItemBreakdown, DrinkItem, DrinkSale, DrinkRestockRequest 
} from '../types';
import { 
  ...
  getDrinks, saveDrinks, getDrinkSales, saveDrinkSales, addDrinkSale, getDrinkRestockRequests, saveDrinkRestockRequests, addDrinkRestockRequest
} from '../data';
import { sendActivityInvoiceViaGmail } from '../utils/formatters';
import { 
  LogOut, UserPlus, Wrench, Plus,  Bed, Calendar, Phone, CheckCircle, Clock, Grid, Filter, 
  Search, Receipt, PlusCircle, Printer, Download, UserCheck, UserMinus, Info, AlertTriangle, Shield, MapPin, X, Sun, Moon, Sliders, RefreshCw, LayoutGrid, Table, Menu, ChevronLeft, ChevronRight, Lock, Unlock, ShieldCheck, Edit2, Wine
} from 'lucide-react';
...
export default function ReceptionistDashboard({ currentUser, onLogout, isDarkMode, onToggleTheme }: ReceptionistDashboardProps) {
  ...
  const handleLockStay = async () => {
    ...
    try {
      ...
      const finalTotalPrice = bookingIsMonthly ? baseRate : (baseRate * days);
  ...
  useEffect(() => {
    ...
    try {
      const roomsQ = query(collection(db, 'rooms'), where('branch', '==', branch));
      unsubRooms = onSnapshot(roomsQ, (snapshot) => {
        if (!snapshot.empty) {
          ...
          setRooms(fetched);
          try {
            const allLocalRooms = getRooms();
            const otherBranchRooms = allLocalRooms.filter(r => r.branch !== branch);
            saveRooms([...otherBranchRooms, ...fetched]);
          } catch (err) {
  ...
  const handleInlineReservationSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if ((resPayment as string) === 'Split') {
        ...
        depositVal = totalPrice * 0.5;
        actualPaymentStatus = 'Partially Paid (50% Deposit)';
  ...
  const handleCreateBookingSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const totalPrice = bookingIsMonthly ? baseRate : (baseRate * diffDays);
  ...
  const handleBookFutureStaySubmit = async (e: React.FormEvent) => {
    ...
    const totalPrice = futureIsMonthly ? baseRate : (baseRate * diffDays);
    ...
    if ((resPayment as string) === 'Split') {
      ...
      finalFuturePaid = futureAmountPaid > 0 ? futureAmountPaid : totalPrice * 0.5;
      actualPaymentStatus = 'Partially Paid (50% Deposit)';
    } else if (resPayment === 'Paid') {
    ...
    if (resPayment !== 'Unpaid' && finalFuturePaid < minDeposit) {
      setFutureStayError(`❌ Minimum 50% deposit (GH₵${minDeposit.toFixed(2)}) is required to lock a future reservation.`);
      return;
    }
    ...
    try {
      ...
      // Store amountPaid attribute
      (newBooking as any).amountPaid = futureAmountPaid;
      (newBooking as any).isFutureBooking = true;
      (newBooking as any).isPartialDeposit = futureAmountPaid < totalPrice;
      ...
      const isPartialDep = futureAmountPaid < totalPrice;
      ...
      await setDoc(doc(db, 'auditLogs', logId), {
        ...
        details: `Created future reservation for "${futureGuestName}" in Room ${room.roomNumber} from ${finalInDate} to ${finalOutDate}. Deposit Paid: GH₵${futureAmountPaid.toFixed(2)}.`
      });
  ...
  const handleConfirmArrivalCheckIn = async () => {
    ...
    try {
      ...
      const nowStr = getFormattedDateTime();
      const depositPaid = Number(arrivalBooking.amountPaid ?? arrivalBooking.deposit ?? arrivalBooking.amountReceived ?? 0);
  ...
  const handleConfirmCheckout = async () => {
    ...
    try {
      ...
      const baseCheckoutTotal = applyLateCheckOutFee ? (selectedBooking.totalPrice + lateCheckOutFee) : selectedBooking.totalPrice;
      ...
      try {
        ...
        const depositPaid = Number(selectedBooking.amountPaid ?? selectedBooking.deposit ?? selectedBooking.amountReceived ?? 0);
        const roomBalanceDue = selectedBooking.totalPrice - depositPaid;
  ...
  const handleCompleteShiftHandover = async () => {
    ...
    const itemsBreakdown: HandoverItemBreakdown[] = [
      ...stats.activeRoomRevs.map((r: any) => {
        ...
        const description = `${prefix}${roomLabel}${typeLabel}${guestLabel}`;
    ...
    <div id="receptionist-dashboard-container" tabIndex={-1} className="bg-zinc-50 dark:bg-zinc-950 text-zinc-900 dark:text-zinc-50 transition-colors duration-200 min-h-screen w-full flex flex-col md:flex-row font-sans outline-none">
        ...
        <div className="p-6 border-b border-zinc-200 dark:border-zinc-800">
          <div className="flex items-center gap-3 mb-4">
            <div 
              ...
              className="w-10 h-10 rounded-xl bg-blue-600 flex items-center justify-center text-white font-extrabold shadow-sm cursor-pointer select-none"
            >
              N
            </div>
            <div>
            ...
            <div className={`border rounded-3xl p-5 shadow-sm ${theme.card}`}>
              ...
              {(() => {
                ...
                const filtered = bookings.filter(b => {
                  // search query
                  if (query) {
                    const matchesSearch = b.guestName.toLowerCase().includes(query) ||
                      b.guestContact.toLowerCase().includes(query) ||
                                  ...
                                  {(b.status === 'Confirmed' || b.status === 'Pending') && (
                                    <>
                                      ...
                                      <button
                                        onClick={() => {
                                          setArrivalBooking(b);
                                          const depositPaid = Number(b.amountPaid ?? b.deposit ?? b.amountReceived ?? 0);
                    ...
                    .map((room) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="flex justify-between items-start">
                            <div>
                              <span className={`text-[10px] font-mono uppercase tracking-wider block ${isDarkMode ? 'text-zinc-500' : 'text-slate-400'}`}>Nabslodge Room</span>
                          ...
                          {isOccupied && activeBooking && (
                            <div className={`border p-3.5 rounded-xl space-y-1.5 text-xs transition-colors ${
                              ...
                              {(() => {
                                ...
                                const roomBalance = Math.max(0, activeBooking.totalPrice - Number(activeBooking.amountPaid ?? activeBooking.deposit ?? 0));
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    {rooms.filter(r => r.status === 'Available').length === 0 && (
                      <p className="text-[10px] text-red-500 mt-1 font-semibold">⚠ No rooms are currently available for booking.</p>
                    )}
                  </div>
                    ...
                    {(() => {
                      const filteredBookings = bookings
                        ...
                        .filter(b => {
                          ...
                          if (!query) return true;
                          const cleanName = b.guestName.replace(/[^\w\d]/g, '').toLowerCase();
                          const cleanContact = b.guestContact.replace(/[^\w\d]/g, '').toLowerCase();
                          const cleanRoom = b.roomNumber.replace(/[^\w\d]/g, '').toLowerCase();
                          return (
                              ...
                              {(b.status === 'Pending' || b.status === 'Confirmed') && (
                                <>
                                  ...
                                  <button
                                    onClick={() => {
                                      setArrivalBooking(b);
                                      const depositPaid = Number(b.amountPaid ?? b.deposit ?? 0);
                                      const totalPrice = Number(b.totalPrice ?? 0);
                                      const bal = totalPrice - depositPaid;
                                      setArrivalAmountReceived(bal);
                                      setShowArrivalModal(true);
                ...
                <div className="p-3.5 bg-blue-500/5 border border-blue-500/15 rounded-2xl">
                  ...
                  {(() => {
                    ...
                    if (checkInDateParsed && checkOutDateParsed && checkOutDateParsed > checkInDateParsed) {
                      ...
                      const totalCost = bookingIsMonthly ? baseRate : (baseRate * days);
                      return (
                        <div className="space-y-4">
                        ...
                        <>
                          ...
                          <div className="pl-2 space-y-0.5 text-[10px] text-zinc-500">
                            {checkoutUnpaidDrinks.map((sale, idx) => (
                              <div key={sale.id || idx} className="flex justify-between">
                    ...
                    <div className="flex w-full gap-2">
                      <button
                        onClick={() => openPrintPreview({
                          ...
                          numberOfNights: Math.max(1, Math.ceil(Math.abs(new Date(selectedBooking.checkOutDate).getTime() - new Date(selectedBooking.checkInDate).getTime()) / (1000 * 3600 * 24))),
                  ...
                  <div className={`border rounded-2xl p-5 space-y-4 text-xs font-sans ${
                    ...
                    <div className={`space-y-2 border-b pb-3 font-mono ${isDarkMode ? 'border-zinc-900' : 'border-slate-200'}`}>
                      <div className="flex justify-between text-[11px]">
                        ...
                        <span className={isDarkMode ? 'text-zinc-300' : 'text-slate-700'}>GH₵{(selectedBooking.totalPrice / Math.ceil((new Date(selectedBooking.checkOutDate).getTime() - new Date(selectedBooking.checkInDate).getTime()) / (1000 * 60 * 60 * 24))).toFixed(2)}</span>
                      </div>
                      <div className="flex justify-between text-[11px]">
                        <span className={isDarkMode ? 'text-zinc-500' : 'text-slate-500'}>Duration (Nights)</span>
                        <span className={isDarkMode ? 'text-zinc-300' : 'text-slate-700'}>x {Math.ceil((new Date(selectedBooking.checkOutDate).getTime() - new Date(selectedBooking.checkInDate).getTime()) / (1000 * 60 * 60 * 24))}</span>
                      </div>
                      <div className="flex justify-between text-[11px]">
                        ...
                        <span className={`uppercase font-bold text-[10px] ${isDarkMode ? 'text-zinc-300' : 'text-slate-700'}`}>
                          GH₵{Number(selectedBooking.deposit ?? selectedBooking.amountPaid ?? (selectedBooking.paymentStatus === 'Paid' ? selectedBooking.totalPrice : (selectedBooking.paymentStatus === 'Partial' || selectedBooking.paymentStatus?.startsWith('Partially')) ? selectedBooking.totalPrice / 2 : 0)).toFixed(2)}
                        </span>
                      </div>
                    ...
                    <div className="flex justify-between items-center pt-1 font-mono">
                      ...
                      <span className="text-base font-black text-emerald-500">
                        GH₵{(() => {
                          const initialPaid = Number(selectedBooking.deposit ?? selectedBooking.amountPaid ?? (selectedBooking.paymentStatus === 'Paid' ? selectedBooking.totalPrice : (selectedBooking.paymentStatus === 'Partial' || selectedBooking.paymentStatus?.startsWith('Partially')) ? selectedBooking.totalPrice / 2 : 0));
                          const baseDues = selectedBooking.totalPrice + (applyLateCheckOutFee ? lateCheckOutFee : 0);
              ...
              <div className="flex-1 overflow-y-auto p-6 pt-2 scrollbar-thin scroll-smooth">
                ...
                {(() => {
                  ...
                  if (futureIsMonthly) {
                    ...
                    baseRate = futureIsOccasion 
                      ? (r.occasionBookingPrice || 1000) 
                      : (r.normalBookingPrice || 600);
                  }
                  const tot = futureIsMonthly ? baseRate : (baseRate * n);
                  const dep = tot * 0.5;
                ...
                {(() => {
                  ...
                  const tot = futureIsMonthly ? baseRate : (baseRate * diffDays);
                  const dep = tot * 0.5;
                    ...
                    <div className="space-y-3 pt-1">
                      <div>
                        ...
                        >
                          ...
                          <option value="Partial">Partial Deposit (Min 50%)</option>
                          <option value="Split">Split Payment (Cash + Momo)</option>
                      ...
                      {resPayment === 'Partial' && (
                        <div>
                          ...
                          <input
                            ...
                            min={dep}
                            step="0.01"
                            placeholder={`Min GH₵${dep.toFixed(2)}`}
      ...
      <AnimatePresence>
        {showArrivalModal && arrivalBooking && (() => {
          ...
          const depositPaid = Number((selectedBooking as any).amountPaid ?? (selectedBooking as any).deposit ?? (selectedBooking as any).amountReceived ?? 0);
      ...
      <AnimatePresence>
        {showPrintInvoiceModal && invoiceBooking && (() => {
          ...
          let paymentsMade = Number(invoiceBooking.amountPaid ?? invoiceBooking.deposit ?? invoiceBooking.amountReceived ?? 0);
          ...
          } else if (invoiceBooking.paymentStatus === 'Partial' || invoiceBooking.paymentStatus?.includes('Partial')) {
            paymentsMade = totalGross / 2;
          } else {
            paymentsMade = Number(invoiceBooking.deposit ?? invoiceBooking.amountPaid ?? 0);
          }
                      ...
                      <div id="print-invoice-sheet" className="printable-invoice-modal space-y-6 print:w-[76mm] print:p-2 print:text-xs print:shadow-none print:space-y-3">
                        ...
                        {invoicePrintShowBranding && (
                          <div className="flex flex-col sm:flex-row justify-between items-start gap-4 pb-6 border-b border-zinc-200 print:flex-col print:gap-1 print:pb-2 print:border-b-dashed print:border-zinc-300">
                            <div>
                              <h1 className="text-3xl font-extrabold tracking-tight text-blue-600 print:text-lg">Nabslodge</h1>
                              <p className="text-xs font-semibold text-zinc-500 uppercase tracking-widest mt-1 print:text-[10px] print:tracking-normal print:mt-0">
                                Nabslodge {branch}
                              </p>
                            ...
                            <table className="w-full text-left text-xs font-mono print:text-[10px]">
                              ...
                              <tbody className="divide-y divide-zinc-200 print:divide-zinc-300">
                                <tr>
                                  ...
                                  <td className="p-3 text-right print:p-1.5">GH₵{nightlyRate.toFixed(2)}</td>
                                  <td className="p-3 text-right font-bold print:p-1.5">GH₵{roomTotal.toFixed(2)}</td>
                                </tr>
          ...
          <div className="fixed inset-0 bg-black/80 flex items-center justify-center p-4 z-50 backdrop-blur-sm">
            ...
            >
              ...
              <p className={`text-sm mb-6 ${isDarkMode ? 'text-zinc-400' : 'text-slate-500'}`}>
                Are you sure you want to end your shift? Please ensure all active check-ins and check-outs have been logged properly.
              </p>
            ...
            >
              ...
              <form onSubmit={handleProcessDrinkSale} className="space-y-4 text-xs">
                ...
                <div className="space-y-1.5">
                  ...
                  {drinkPaymentMethod === 'Split (Paid & Unpaid)' && (
                    <div className={`grid grid-cols-2 gap-3 mt-3 p-3 rounded-xl border ${isDarkMode ? 'bg-purple-500/5 border-purple-500/20' : 'bg-purple-50 border-purple-100'}`}>
                      <div className="flex flex-col justify-end">
                      ...
                      <div className="flex flex-col justify-end">
                        <label className={`block text-[10px] font-mono mb-1 leading-tight ${isDarkMode ? 'text-zinc-400' : 'text-slate-500'}`}>Unpaid / Room Bill (GH₵)</label>
                        <input
                          type="number"
                          step="0.01"
